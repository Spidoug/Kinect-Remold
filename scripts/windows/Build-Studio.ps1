...
# cmd.exe/WSH launchers are intentionally ASCII + CRLF with no BOM.  Re-emit
# both files instead of trusting editor, Git or ZIP newline/encoding behavior.
foreach($launcherName in @('SynKinectStudio.cmd','SynKinectStudio.vbs')){
  $launcherPath=Join-Path $WindowsApp $launcherName
  $launcherText=[IO.File]::ReadAllText($launcherPath,[Text.Encoding]::UTF8) -replace "`r?`n","`r`n"
  if($launcherText.ToCharArray() | Where-Object{[int]$_ -gt 127} | Select-Object -First 1){throw "Windows launcher contains non-ASCII text: $launcherName"}
  [IO.File]::WriteAllText($launcherPath,$launcherText,[Text.Encoding]::ASCII)
  $bytes=[IO.File]::ReadAllBytes($launcherPath)
  if($bytes.Length -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF){throw "Unsafe UTF-8 BOM remained in $launcherName"}
}
