...
if (!(Test-Path -LiteralPath $RuntimeJava -PathType Leaf)) { throw "Package validation failed: $RuntimeJava is missing." }
...
foreach ($relative in $required) {
  ...
  if (!(Test-Path -LiteralPath $path -PathType Leaf)) { throw "Package validation failed: missing $relative" }
}
