...
The Media Foundation virtual camera remains NV12 because that is its Windows-facing format. Its RGB conversion is isolated from ScannerPort and never controls the physical stream mode.
...
The RAW camera interface keeps conversion after the ScannerPort boundary: inside SynKinect Studio for application processing, or inside an explicit operating-system/network adapter that must produce NV12, YUYV or JPEG.
