...
Model 1473 startup waits up to five seconds for the post-firmware audio/control sibling and issues its semantic LED keep-alive. Isochronous fixed-size streams use a bounded short-burst recovery window. Missing or short payload regions are cleared before a frame is published, avoiding stale pixels while preserving RGB, IR and Depth cadence on a busy USB 2 controller.
