...
The native audio runtime owns one USB Audio capture session for each physical Kinect and publishes raw 4-channel S32LE/16 kHz frames tagged by the stable Kinect ID. The Studio subscribes to the selected sensor. Every 256-sample block is processed by the acoustic worker; analysis is not tied to the UI frame rate.
