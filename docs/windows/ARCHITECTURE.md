...
Model 1473 is a distinct physical backend behind the same public control interface as model 1414. Its `02AE` camera keeps the same product ID as the 1414 camera but reports a different USB device revision; CameraBridge keeps the same per-device discovery/pipe architecture. LED, tilt and accelerometer commands use the alternate bulk protocol on `02BB/02C3&MI_00` (`OUT 0x01`, `IN 0x81`), while `MI_02` remains owned by Microsoft USB Audio/WASAPI. The Broker does not generate cosmetic LED or motor traffic. Explicit control transactions receive one fresh-handle retry and do not reset/flush the `MI_00` pipes. Scanner motion metadata uses the same logical status interface on both models; 1414 polls the dedicated `02B0` path at 25 ms, while 1473 uses a conservative 250 ms cadence on `MI_00` so the sibling audio runtime is not hammered.

The audio path is therefore:

`02AD boot/WinUSB → Microsoft UACFirmware 01.02.709.00 upload → 02BB/02C3 MI_02/USB Audio → one WASAPI capture session per physical Kinect → multi-client raw 4-channel bus `Kinect360RemoldAudio` keyed by Kinect ID → independent SynKinect Studio DSP instances (Microphones + Acoustic Scanner)`
...
The public microphone interface is: 4 channels, 16 kHz, 32-bit signed little-endian PCM, 256 samples/channel, local named pipe `\\.\pipe\Kinect360RemoldAudio`. A subscription includes the stable physical Kinect ID so simultaneous sensors remain independent on the shared transport.
