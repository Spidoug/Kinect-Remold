...
The AudioBridge uses overlapped `ConnectNamedPipe`. Its completion path must always pass a valid transferred-byte storage pointer to `GetOverlappedResult`; using a null pointer can make an otherwise valid asynchronous client connection fail before the protocol handshake. V1 publishes one canonical multi-client raw-audio endpoint, `\\.\pipe\Kinect360RemoldAudio`; each subscription selects a physical Kinect ID. Microphones and Acoustic open independent client sessions on that bus, so modules and simultaneous sensors do not own or starve one another.
