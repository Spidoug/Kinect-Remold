...
- `runtime_capture=wasapi-shared-windows-audio`
- `firmware_kind=Microsoft-Kinect-Runtime-1.8-UACFirmware`
- `firmware_version=01.02.709.00`
- `uac_profile=microsoft-runtime-native-shared`
- `uac_iso_in_endpoint=0x82`
- `uac_iso_in_binterval=device-firmware-defined`
...
- `uac-runtime-reconnecting`: the physical Kinect is present but its matching Windows USB Audio endpoint is not ready yet; this is the normal temporary state during 02AD → 02BB/02C3 re-enumeration for either 1414 or 1473.
...
- `uac-runtime-capturing`: expected steady state. `capture_mode=wasapi-shared-native-input`; `published_frames` should increase, the Windows Sound input meter should remain live, and a Processing subscriber should increase `pipe_frames`.


## Service lifetime

AudioBridge is a persistent Win32 product runtime, not merely a side effect of the transient `02AD` boot INF. Install/Reinstall copies the current built executable to `%ProgramFiles%\Kinect Xbox 360 Remold`, creates or reconfigures the service even when the Kinect is already in post-firmware `02BB/02C3`, reapplies that persistent `ImagePath` after any `02AD` INF binding, uses delayed automatic start, and configures restart-on-failure. The INF can still create/start the same service when `02AD` is present, but Microphones/Acoustic use the persistent raw-audio service across normal runtime re-enumeration. This is essential because UAC firmware is RAM-resident and `02AD` disappears after firmware launch.

The **Microphones** tab in `SynKinectStudio` opens only the endpoint advertised for the selected Kinect: `\\.\pipe\Kinect360RemoldAudio-<device-id>`. Physical presence creates that per-device endpoint even while Windows is still restoring the UAC capture endpoint, so both 1414 and 1473 visibly report reconnecting instead of one model disappearing from the audio UI. The runtime never falls back to another Kinect or to an unrelated endpoint.


## Named-pipe connection reliability

The AudioBridge uses overlapped `ConnectNamedPipe`. Its completion path must always pass a valid transferred-byte storage pointer to `GetOverlappedResult`; using a null pointer can make an otherwise valid asynchronous client connection fail before the protocol handshake. The runtime publishes one canonical multi-client raw-audio endpoint **per Kinect**, `\\.\pipe\Kinect360RemoldAudio-<device-id>`; Microphones and Acoustic open independent client sessions on the selected Kinect's latest-frame bus, so one module does not own or starve the other and one Kinect cannot steal another Kinect's clients.
