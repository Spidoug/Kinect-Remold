...
Runtime services own physical Kinect transports and expose stable per-device application endpoints. `/run/kinect360-remold/devices.tsv` publishes:

`id, label, state, control, camera, audio, audio-control, virtual-camera, sdk`.

Each Kinect receives its own camera socket, audio socket, audio-control socket and V4L2 virtual-camera assignment. The control ABI carries the same `deviceId`, so Tilt/LED/status operations cannot fall through to another connected sensor. The manifest probes control availability with a non-invasive `Ping`; it does not poll accelerometer/motor status merely to keep the UI enabled. The audio-control socket is published as a per-device capability independently of the instantaneous ALSA capture state.
