# Linux libusb-1.0 Kinect 1414/1473 backend

## Purpose

`drivers/linux/source/src/kinect_usb_camera.cpp` implements the Kinect v1 camera transport directly on libusb-1.0 in user space over Linux usbfs; udev supplies device permissions and hot-plug policy.
...
The backend uses multiple asynchronous libusb transfers to keep the isochronous queue populated. Each transfer slot is sized from `libusb_get_max_iso_packet_size()` for the active endpoint, while the Kinect frame parser independently enforces the sensor's fixed logical packet framing (1920 bytes including header for video, 1760 for Depth). Keeping transport capacity separate from protocol payload avoids under-scheduling high-bandwidth USB endpoints. A USB removal or transfer failure that cannot be resubmitted invalidates only that camera session instead of disturbing audio/control.
