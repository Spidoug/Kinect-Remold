...
it is older than 250 ms. The Linux Broker exchange is bounded to 2000 ms by
socket timeouts. Polling is serialized, so a slower 1473 transaction cannot
overlap the next 25 ms request; the next period starts after the current
