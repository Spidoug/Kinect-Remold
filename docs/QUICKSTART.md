...
The Studio discovers devices through the Broker SDK endpoint. A Kinect remains represented while it is `Booting` or `Reconnecting`; controls are enabled only when their required endpoint reports ready.
