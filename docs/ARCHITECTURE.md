...
Windows and Linux use the same end-to-end runtime architecture. Platform differences stop at the native transport adapters; they do not change ownership, lifecycle, discovery, device identity or application contracts.
...
Kinect 1414 and 1473 share the same Kinect Xbox 360 camera RGB/IR engine behavior. Remold therefore rejects requests that would require RGB and IR simultaneously. Depth is independent and can run alongside either video mode.
...
Public Linux SDK contracts are published as `Kinect360RemoldControlProtocol.hpp` and `Kinect360RemoldAudioControlProtocol.hpp`. ScannerPort, raw audio transport and runtime paths remain internal.
