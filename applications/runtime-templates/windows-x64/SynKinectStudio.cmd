...
rem Direct launches normally hand off to the windowless WScript launcher.
rem Validate the VBS first: UTF-8 BOM/corruption in WSH otherwise produces the
rem misleading "line 1, char 1: invalid character" failure.
rem REMOLD_STUDIO_HIDDEN is set by SynKinectStudio.vbs to prevent recursion.
if not defined REMOLD_STUDIO_HIDDEN if exist "%~dp0SynKinectStudio.vbs" (
  set "REMOLD_VBS_PATH=%~dp0SynKinectStudio.vbs"
  powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:REMOLD_VBS_PATH; try{$b=[IO.File]::ReadAllBytes($p); if($b.Length -ge 3 -and $b[0] -eq 0xEF -and $b[1] -eq 0xBB -and $b[2] -eq 0xBF){exit 2}; $t=[IO.File]::ReadAllText($p,[Text.Encoding]::ASCII); if($t.TrimStart().StartsWith('Option Explicit')){exit 0}; exit 3}catch{exit 4}" >nul 2>&1
  if not errorlevel 1 (
    wscript.exe //nologo "%~dp0SynKinectStudio.vbs" %* >nul 2>&1
    exit /b 0
  )
  set "REMOLD_STUDIO_HIDDEN=1"
)

rem SynKinect Studio is always a standard-user desktop process.
rem If the launcher inherits an elevated control token, hand execution to the
rem normal Explorer desktop token before Java starts.
...
if "%_REMOLD_IS_ADMIN%"=="1" (
  ...
  powershell.exe -NoLogo -NoProfile -Command "$sh=New-Object -ComObject Shell.Application; $sh.ShellExecute($env:REMOLD_STUDIO_SCRIPT,'--remold-standard-user',$env:REMOLD_STUDIO_DIR,'open',0)"
  if errorlevel 1 (
...
rem Runtime rule: use one Java 17+ runtime. A bundled runtime wins when present.
...
if not defined JAVA_GUI_EXE set "JAVA_GUI_EXE=%JAVA_EXE%"
"%JAVA_GUI_EXE%" -Dfile.encoding=UTF-8 -Dsun.java2d.uiScale=1.0 -Dsun.java2d.dpiaware=true -cp "%APP_CP%" SynKinectStudio %* >>"%LOG_FILE%" 2>&1
...
set "JAVA_GUI_EXE=%CANDIDATE%"
for %%G in ("%CANDIDATE%") do if /I "%%~nxG"=="java.exe" if exist "%%~dpGjavaw.exe" set "JAVA_GUI_EXE=%%~dpGjavaw.exe"
