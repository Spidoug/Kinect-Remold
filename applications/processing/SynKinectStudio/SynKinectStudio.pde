...
void drawStudioFatalScreen(){
  ...
  fill(0xFFAAB6C2);textSize(14);text("Close this window after copying the log path if you need to report the failure.",54,height-48);
...
class KinectDeviceRegistry {
  ...
  ArrayList<KinectDevice> querySdk()throws IOException{
    ArrayList<KinectDevice> found=new ArrayList<KinectDevice>();
    TransportEndpoint endpoint=studio.services.endpoints.sdk;
  ...
  void refresh(){
    ...
    synchronized(lock){
      ...
      for(KinectDevice old:devices){
        Long seen=lastSeenMs.get(old.id);
        if(!merged.containsKey(old.id)&&seen!=null&&now-seen<disappearanceGraceMs){
          merged.put(old.id,new KinectDevice(old.id,old.label,"Reconnecting","","","","","",""));
        }
      }
      ...
      Collections.sort(devices,new Comparator<KinectDevice>(){public int compare(KinectDevice a,KinectDevice b){return a.id.compareTo(b.id);}});
      boolean selectedPresent=false;
      for(KinectDevice d:devices)if(d.id.equals(selectedId)){selectedPresent=true;break;}
  ...
  void cycle(){synchronized(lock){if(devices.isEmpty()){selectedId="";return;}int idx=0;
      for(int i=0;i<devices.size();i++)if(devices.get(i).id.equals(selectedId)){idx=i;break;}selectedId=devices.get((idx+1)%devices.size()).id;generation++;}}
  String selectorLabel(){synchronized(lock){if(devices.isEmpty())return "Kinect · 0";
      int idx=0;for(int i=0;i<devices.size();i++)if(devices.get(i).id.equals(selectedId)){idx=i;break;}KinectDevice d=devices.get(idx);String name=d.label==null||d.label.trim().isEmpty()?"Kinect":d.label.trim();
...
class StudioEndpoints {
  final TransportEndpoint sdk=new TransportEndpoint("\\\\.\\pipe\\Kinect360RemoldSdk","/run/kinect360-remold/sdk.sock","sdk");
