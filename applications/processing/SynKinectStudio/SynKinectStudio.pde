...
final int STUDIO_TAB_H=50;
final int STUDIO_MIN_WIDTH=1280;
final int STUDIO_MIN_HEIGHT=800;

// Shared visual system used by every Studio module.
final int STUDIO_UI_MARGIN=20;
final int STUDIO_UI_GAP=14;
final int STUDIO_UI_RADIUS=14;
final int STUDIO_UI_HEADER_H=68;
final int STUDIO_UI_CARD_TITLE_H=44;
final int STUDIO_UI_BUTTON_H=42;
final int STUDIO_UI_BUTTON_MIN_H=38;
...
volatile boolean studioNativeCloseRequested=false;
...
void initializeStudioTypography(){
  ...
  boolean japanese=locale!=null&&locale.toLowerCase(Locale.ROOT).startsWith("ja");
  String regular=installedStudioFont(japanese
    ?new String[]{"Yu Gothic UI","Meiryo UI","Noto Sans CJK JP","Noto Sans JP","Noto Sans","DejaVu Sans","Dialog"}
    :new String[]{"Segoe UI Variable Text","Segoe UI Variable","Segoe UI","Inter","Noto Sans","Cantarell","Ubuntu Sans","Ubuntu","DejaVu Sans","Liberation Sans","Dialog"});
  String heading=installedStudioFont(japanese
    ?new String[]{"Yu Gothic UI Semibold","Yu Gothic UI","Meiryo UI","Noto Sans CJK JP","Noto Sans JP","Dialog"}
    :new String[]{"Segoe UI Variable Display","Segoe UI Variable","Segoe UI Semibold","Segoe UI","Inter SemiBold","Inter","Noto Sans SemiBold","Noto Sans","Cantarell","Ubuntu Sans","Ubuntu","DejaVu Sans","Dialog"});
...
public void draw(){
  enforceStudioMinimumWindow();
  if(studioNativeCloseRequested&&!studioExitCommitted){
    studioNativeCloseRequested=false;
    exit();
...
public void exit(){
  // NEWT delivers title-bar close notifications on its own event thread on
  // Linux. Never tear modules down from that thread: the listener below only
  // posts a flag and this method is reached from Processing's animation thread.
  if(studioExitCommitted)return;
  if(studio.rejectExitRequest()){studioNativeCloseRequested=false;return;}
...
void enforceStudioMinimumWindow(){
  ...
  catch(Exception e){println("Studio minimum-size warning: "+safeStudioMessage(e));}
}
    ...
    if(nativeWindow instanceof GLWindow){
      ...
      studioCloseListener=new WindowAdapter(){
        @Override public void windowDestroyNotify(WindowEvent event){
          // Cross-thread handoff only. draw()/exit() performs the actual orderly
          // shutdown through the bounded lifecycle path.
          studioNativeCloseRequested=true;
        }
      };
...
class ScannerStudioModule extends StudioModuleBase {
  ...
  public void activateModule(){
    ...
    if(owner.scannerState.source!=null){
      // Scanner can request the native 1280x1024 Bayer stream for scan capture.
      owner.scannerState.source.setHqColorRequested(owner.scannerState.config.rgbHqEnabled);
  ...
  public void deactivateModule(){
    ...
    if(owner.scannerState.scanActive){
      owner.scannerState.scanPaused=true;clearPendingScanWork();
    }
    if(owner.scannerState.source!=null){
...
class StudioController {
  ...
  void onDeviceRegistryChanged(){
    ...
    if(acousticState.source!=null)acousticState.source.requestReconnect("device-registry");
    if(microphoneState.source!=null)microphoneState.source.requestReconnect("device-registry");
  ...
  boolean homeButtonHit(float mx,float my){return mx>=homeButtonX()&&mx<=homeButtonX()+homeButtonW()&&my>=7&&my<=43;}
  float tabWidth(){
  ...
  boolean languageButtonHit(float mx,float my){return mx>=languageButtonX()&&mx<=languageButtonX()+languageButtonW()&&my>=7&&my<=43;}
  boolean deviceButtonHit(float mx,float my){return mx>=deviceButtonX()&&mx<=deviceButtonX()+deviceButtonW()&&my>=7&&my<=43;}
  ...
  void drawTabs(){
    ...
    stroke(homeHot?0xFF68A9E8:0xFF35414D);fill(homeActive?0xFF293440:(homeHot?0xFF222B34:0xFF181E25));rect(homeButtonX(),7,homeW,36,9);noStroke();
    fill(homeActive?0xFFF4F7FA:0xFFAAB6C2);textAlign(CENTER,CENTER);studioText(STUDIO_FONT_BUTTON,true);String homeLabel=studio.i18n.tr("button.home");fitCurrentTextSize(homeLabel,STUDIO_FONT_BUTTON,8,max(10,homeW-10),30);text(ellipsizeToWidth(homeLabel,max(10,homeW-10)),homeButtonX()+homeW/2,25);
    float tabW=tabWidth(),x=tabsStartX();
    for(int i=0;i<modules.length;i++){
      boolean active=i==activeTab;
      boolean moduleReady=isReady(i);
      fill(active?0xFF293440:0xFF181E25);rect(x,7,tabW,36,9);
      fill(active?0xFFF4F7FA:0xFFAAB6C2);textAlign(CENTER,CENTER);
      studioText(STUDIO_FONT_BUTTON,true);fitCurrentTextSize(modules[i].title(),STUDIO_FONT_BUTTON,8,max(12,tabW-20),30);text(ellipsizeToWidth(modules[i].title(),max(12,tabW-20)),x+tabW/2,25);
      if(active&&!moduleReady){fill(0xFF78B7E8);ellipse(x+tabW-9,25,5,5);}
      x+=tabW+gap;
    }
    float dx=deviceButtonX(),dw=deviceButtonW();boolean dhot=deviceButtonHit(mouseX,mouseY);
    stroke(dhot?0xFF68A9E8:0xFF35414D);fill(dhot?0xFF293440:0xFF181E25);rect(dx,7,dw,36,9);noStroke();fill(services.devices.count()>0?0xFFF4F7FA:0xFF7E8994);
    String deviceLabel=services.devices.selectorLabel();textAlign(CENTER,CENTER);studioText(STUDIO_FONT_BUTTON,true);fitCurrentTextSize(deviceLabel,STUDIO_FONT_BUTTON,8,dw-12,30);text(ellipsizeToWidth(deviceLabel,dw-12),dx+dw/2,25);
    float lx=languageButtonX(),lw=languageButtonW();boolean hot=languageButtonHit(mouseX,mouseY);
    stroke(hot?0xFF68A9E8:0xFF35414D);fill(hot?0xFF293440:0xFF181E25);rect(lx,7,lw,36,9);noStroke();fill(0xFFF4F7FA);
    String lang=studio.i18n.tr("button.language")+" · "+studio.i18n.shortLanguage();textAlign(CENTER,CENTER);studioText(STUDIO_FONT_BUTTON,true);fitCurrentTextSize(lang,STUDIO_FONT_BUTTON,8,lw-12,30);text(ellipsizeToWidth(lang,lw-12),lx+lw/2,25);
  ...
  int tabAt(float mx,float my){
    ...
    for(int i=0;i<modules.length;i++){if(mx>=x&&mx<=x+tabW&&my>=7&&my<=43)return i;x+=tabW+gap;}
    return -1;
  }
  ...
  void drawCloseGuardNotice(){
    ...
    fill(0xFFF4F7FA);textAlign(CENTER,CENTER);studioText(STUDIO_FONT_TINY,false);fitCurrentTextSize(msg,STUDIO_FONT_TINY,8,w-24,h-8);text(ellipsizeToWidth(msg,w-24),x+w/2,y+h/2);textAlign(LEFT,BASELINE);
  }
  ...
  void dispose(){
    ...
    boolean operationHeld=false;
    try{
      // A broken USB/native call must never pin the Linux window forever. Give
      // an in-flight tab transition a bounded grace period, then continue with
      // idempotent module stop methods so Processing can destroy the GL window.
      operationHeld=operationLock.tryLock(900,java.util.concurrent.TimeUnit.MILLISECONDS);
      if(!operationHeld)println("Studio shutdown: lifecycle operation did not quiesce; forcing bounded module teardown.");
...
void drawStudioHome(){
  ...
  fill(0xFFF4F7FA);textAlign(CENTER,TOP);studioText(30,true);text("SynKinect Studio",width/2,42);
  fill(0xFF8FA2B4);studioText(STUDIO_FONT_BODY,false);String homeSub=studio.i18n.tr("home.subtitle");fitCurrentTextSize(homeSub,STUDIO_FONT_BODY,9,width-48,26);text(ellipsizeToWidth(homeSub,width-48),width/2,86);
  fill(0xFFAAB6C2);studioText(STUDIO_FONT_SMALL,false);String homeDesc=studio.i18n.tr("home.description");fitCurrentTextSize(homeDesc,STUDIO_FONT_SMALL,8,width-64,24);text(ellipsizeToWidth(homeDesc,width-64),width/2,114);
  ...
  for(int i=0;i<studio.modules.length;i++){
    ...
    stroke(hot?0xFF68A9E8:0xFF2D3945);fill(hot?0xFF1F2A34:0xFF171E25);rect(x,y,cardW,cardH,12);noStroke();fill(0xFFF4F7FA);textAlign(LEFT,TOP);studioText(STUDIO_FONT_LABEL,true);String cardTitle=studio.modules[i].title();fitCurrentTextSize(cardTitle,STUDIO_FONT_LABEL,9,cardW-32,24);text(ellipsizeToWidth(cardTitle,cardW-32),x+16,y+14);fill(0xFF91A0AE);studioText(STUDIO_FONT_TINY,false);text(homeModuleDescription(i),x+16,y+42,cardW-32,max(22,cardH-48));
  }
  float panelY=homeModuleEndY()+max(28,36*studioUiScale());studio.homeSystemPanel.draw(cx,panelY,cw,max(110,studio.contentHeight-panelY-34));
...
class StudioHomeSystemPanel {
  final String[] actions={"Install","Status","OpenCamera","Tilt","StartupTilt","IpStatus","IpReset","IpToggle","RgbHqToggle","OpenStudio","Uninstall"};
  ...
  void draw(float x,float y,float w,float availableH){
    float innerW=min(w,1040*studioUiScale()),innerX=x+(w-innerW)*0.5f;
    fill(0xFFE7EDF3);textAlign(CENTER,TOP);studioText(STUDIO_FONT_BODY,true);String title=studio.i18n.tr("system.title");fitCurrentTextSize(title,STUDIO_FONT_BODY,9,innerW,24);text(ellipsizeToWidth(title,innerW),x+w*0.5f,y);
    fill(0xFF8999A8);studioText(STUDIO_FONT_TINY,false);String description=studio.i18n.tr("system.description");fitCurrentTextSize(description,STUDIO_FONT_TINY,8,innerW,22);text(ellipsizeToWidth(description,innerW),x+w*0.5f,y+24);
    int cols=innerW>=940?5:(innerW>=620?3:2),rows=(actions.length+cols-1)/cols;float gap=max(6,9*studioUiScale()),top=y+54,bh=constrain((availableH-78-gap*(rows-1))/max(1,rows),STUDIO_UI_BUTTON_MIN_H,44),bw=(innerW-gap*(cols-1))/cols;
    for(int i=0;i<actions.length;i++){int row=i/cols,col=i%cols;float bx=innerX+col*(bw+gap),by=top+row*(bh+gap);buttons[i].set(bx,by,bw,bh);boolean enabled=studio.systemControl.available(),hot=enabled&&buttons[i].hit(studio.contentMouseX(),studio.contentMouseY());stroke(hot?0xFF68A9E8:0xFF35414D);fill(enabled?(hot?0xFF263542:0xFF1A232B):0xFF14191E);rect(bx,by,bw,bh,8);noStroke();fill(enabled?0xFFE9F0F5:0xFF59636D);textAlign(CENTER,CENTER);studioText(STUDIO_FONT_BUTTON,true);String text=label(actions[i]);fitCurrentTextSize(text,STUDIO_FONT_BUTTON,8,bw-10,bh-6);text(ellipsizeToWidth(text,bw-10),bx+bw/2,by+bh/2);}
    fill(studio.systemControl.lastOk?0xFF7CC7A0:0xFFE4B86B);studioText(STUDIO_FONT_TINY,false);textAlign(CENTER,TOP);String state=studio.systemControl.message();text(ellipsizeToWidth(state,innerW),x+w*0.5f,min(studio.contentHeight-42,top+rows*(bh+gap)+3));textAlign(LEFT,BASELINE);
  }
...
class StudioSystemControl {
  ...
  void run(final String action){
    ...
    lastOk=true;lastMessage=studio.i18n.format("system.started",studio.i18n.tr("system."+action.toLowerCase(Locale.ROOT)));final File target=script;final String deviceId=selectedKinectDeviceId();
    studio.services.workers.startLowPriority("System-Control-"+action,new Runnable(){public void run(){try{
      if(studio.services.transportFactory.isWindows()){
        String childArgs="@('-NoLogo','-NoProfile','-ExecutionPolicy','Bypass','-File',"+psLiteral(target.getAbsolutePath())+",'-Action',"+psLiteral(action)+(deviceId.length()>0?",'-DeviceId',"+psLiteral(deviceId):"")+")";
        ...
        String command="REMOLD_GUI=1 bash "+shQuote(target.getAbsolutePath())+" --action "+shQuote(action)+(deviceId.length()>0?" --device "+shQuote(deviceId):"")+"; rc=$?; printf '\\n'; read -r -p 'Press Enter to close: ' _ || true; exit $rc";
...
void drawModuleStartup(StudioModuleBase module){
  ...
  fitCurrentTextSize(message,STUDIO_FONT_SMALL,8,width-48,30);text(ellipsizeToWidth(message,width-48),width*0.5f,max(120,(height-STUDIO_TAB_H)*0.42f+42));
  textAlign(LEFT,BASELINE);
}
...
class StudioEndpoints {
  ...
  final TransportEndpoint audioControl=new TransportEndpoint("\\\\.\\pipe\\Kinect360RemoldAudioControl","/run/kinect360-remold/audio-control.sock","audio-control");
  final TransportEndpoint skeleton=new TransportEndpoint("\\\\.\\pipe\\Kinect360RemoldNuiSkeleton","/run/kinect360-remold/nui-skeleton.sock","nui-skeleton");
