...
boolean loadScannerRgbHqPreference(boolean fallback){
  try{return java.util.prefs.Preferences.userRoot().node("kinect360-remold/synkinect-studio").getBoolean("scanner.rgbHqEnabled",fallback);}
  catch(Exception ignored){return fallback;}
}
void saveScannerRgbHqPreference(boolean enabled){
  try{java.util.prefs.Preferences.userRoot().node("kinect360-remold/synkinect-studio").putBoolean("scanner.rgbHqEnabled",enabled);}
  catch(Exception ignored){}
}

void ensureRgbdCore(){
  if(studio.scannerState.config==null){studio.scannerState.config=new AppConfig();studio.scannerState.config.load(new File(dataPath("scanner.properties")));studio.scannerState.config.rgbHqEnabled=loadScannerRgbHqPreference(studio.scannerState.config.rgbHqEnabled);}
...
void toggleScannerRgbHq(){
  if(studio.scannerState.scanActive||studio.scannerState.resetBusy||studio.scannerState.meshBusy){
    studio.scannerState.status=studio.scannerState.i18n.tr("status.rgb_hq_stop_scan");return;
  }
  studio.scannerState.config.rgbHqEnabled=!studio.scannerState.config.rgbHqEnabled;
  saveScannerRgbHqPreference(studio.scannerState.config.rgbHqEnabled);
  if(studio.scannerState.source!=null)studio.scannerState.source.setHqColorRequested(studio.scannerState.config.rgbHqEnabled);
  studio.scannerState.status=studio.scannerState.i18n.tr(studio.scannerState.config.rgbHqEnabled?"status.rgb_hq_enabled":"status.rgb_hq_disabled");
}
...
void dispatchUiAction(int action) {
  ...
  else if(action==studio.scannerState.ui.ACTION_RGB_HQ)toggleScannerRgbHq();
...
void scannerKeyPressed() {
  ...
  if (key == 'c' || key == 'C') cleanMesh(); if (key == 'f' || key == 'F') smoothMesh(); if (key == 'x' || key == 'X') centerMesh(); if (key == 'k' || key == 'K') toggleDepthCalibration(); if (key == 'h' || key == 'H') toggleScannerRgbHq();
...
void reconstructionLoop() {
  while (true) {
    ...
    if (work != null) {
      ...
      finally{studio.scannerState.reconstructionBusy=false;synchronized(studio.scannerState.reconstructionQueueLock){studio.scannerState.reconstructionQueueLock.notifyAll();}}
    }
  }
  studio.scannerState.reconstructionBusy=false;
}
...
class UiTheme {
  ...
  final int RADIUS = STUDIO_UI_RADIUS;
  final int MARGIN = STUDIO_UI_MARGIN;
  final int GAP = STUDIO_UI_GAP;
  final int HEADER_H = STUDIO_UI_HEADER_H;
  final int TOOLBAR_H = 132;
  final int SIDEBAR_W = 430;
  final int CARD_TITLE_H = STUDIO_UI_CARD_TITLE_H;
  ...
  PointCloud build(DepthFrame f, Calibration c, int step, float targetZ, float band, PointCloudBuildStats stats, RgbSnapshot rgb) {
    ...
    for (int v = 0; v < f.height; v += safeStep) {
      ...
      for (int u = 0; u < f.width; u += safeStep) {
        ...
        if(confidence<cfg.pointCloudMinimumConfidence){if(stats!=null)stats.rejectedSpatial++;continue;}
...
class MultiFrameDepthSuperResolver {
  ...
  PointCloud fuse(ArrayList<HighQualityKeyframe> frames,ArrayList<RigidTransform> poses,int anchorIndex,RgbSnapshot rgb){
    ...
    for(int j=from;j<=to;j++){
      ...
      for(int v=0;v<f.height;v++){int row=v*f.width;for(int u=0;u<f.width;u++){int index=row+u,raw=f.depth[index]&0xffff;if(raw==0)continue;int mm=builder.filteredDepthMmHighQuality(f,u,v,raw,calibration);if(mm<=0)continue;float z=mm*calibration.depthScale;if(z<cfg.minDepthM||z>cfg.maxDepthM)continue;if(!Float.isNaN(k.targetDepthM)&&abs(z-k.targetDepthM)>k.bandM)continue;
        float conf=calibration.depthConfidence(index,z)*builder.edgeConfidence(f,u,v,mm,calibration);if(conf<max(cfg.pointCloudMinimumConfidence,0.22f))continue;stats.sourceSamples++;
        ...
        float rangeWeight=1.0f/(1.0f+0.30f*z*z),baseWeight=max(0.05f,conf*rangeWeight);float sx=(pu+0.5f)*scale-0.5f,sy=(pv+0.5f)*scale-0.5f;splat(sx,sy,az,baseWeight,tag);
      }}
    }
...
class ScannerUI {
  ...
  final int ACTION_STL=4, ACTION_OBJ=5, ACTION_PLY=6, ACTION_CLEAN=7, ACTION_SMOOTH=8, ACTION_CENTER=9, ACTION_UNDO=10, ACTION_CALIBRATE=11, ACTION_RGB_HQ=12;
 ...
 final ArrayList<UiActionGroup> groups = new ArrayList<UiActionGroup>();
  ...
  ScannerUI() {
    ...
    capture.add(button("button.rgb_hq", ACTION_RGB_HQ, BUTTON_NORMAL));
  ...
  String rgbFooterValue(){
    ...
    if(studio.scannerState.config.rgbHqEnabled)frames+="  ·  RGB HQ";
  ...
  void drawToolbar(float x,float y,float w,float h) {
    float[] weights={0.38f,0.38f,0.24f};
  ...
  void drawActionPanel(UiActionGroup group,float x,float y,float w,float h) {
    ...
    float bx=x+10, gap=7, contentY=y+32, contentH=max(STUDIO_UI_BUTTON_MIN_H,h-42);
    int count=max(1,group.items.size());
    int cols=count>=5?(w>=300?3:2):count;
    int rows=(count+cols-1)/cols;
    float bw=(w-20-gap*(cols-1))/cols;
    float bh=constrain((contentH-gap*(rows-1))/rows,STUDIO_UI_BUTTON_MIN_H,44);
    float gridH=rows*bh+gap*(rows-1),by=contentY+max(0,(contentH-gridH)*0.5f);
    for(int i=0;i<group.items.size();i++) {
      int row=i/cols,col=i%cols;
      UiButton item=group.items.get(i);
      item.setBounds(bx+col*(bw+gap),by+row*(bh+gap),bw,bh);
      item.draw();
    }
  }
...
class UiButton {
  ...
  boolean enabled(){
    ...
    if(action==studio.scannerState.ui.ACTION_CALIBRATE)return studio.scannerState.config.calibrationEnabled&&!studio.scannerState.scanActive&&studio.scannerState.latestDepth!=null;
    if(action==studio.scannerState.ui.ACTION_RGB_HQ)return !studio.scannerState.scanActive&&!studio.scannerState.meshBusy;
  ...
  void draw(){
    ...
    boolean selected=(action==studio.scannerState.ui.ACTION_RGB_HQ&&studio.scannerState.config.rgbHqEnabled)
      ||(action==studio.scannerState.ui.ACTION_CALIBRATE&&studio.scannerState.calibrationSession!=null&&studio.scannerState.calibrationSession.active)
      ||(action==studio.scannerState.ui.ACTION_PAUSE&&studio.scannerState.scanActive&&studio.scannerState.scanPaused);
    ...
    stroke((hot||selected)?studio.services.uiTheme.ACCENT:studio.services.uiTheme.BORDER); strokeWeight(1);
    fill(en?(selected?studio.services.uiTheme.ACCENT_SOFT:(hot?studio.services.uiTheme.SURFACE_RAISED:base)):studio.services.uiTheme.BG);
    rect(x,y,w,h,8); noStroke();
    fill(en?(selected||style==studio.scannerState.ui.BUTTON_PRIMARY?studio.services.uiTheme.TEXT:studio.services.uiTheme.TEXT_MUTED):studio.services.uiTheme.BORDER);
    textAlign(CENTER,CENTER); uiText(STUDIO_FONT_BUTTON,true);String value=label();fitCurrentTextSize(value,STUDIO_FONT_BUTTON,8,w-12,h-8);text(ellipsizeToWidth(value,w-12),x+w/2,y+h/2); textAlign(LEFT,BASELINE);
  }
