...
class SurveillanceUI {
  ...
  void draw(){SurveillanceModuleState s=studio.surveillanceState;float m=studio.services.surveillanceTheme.MARGIN,g=studio.services.surveillanceTheme.GAP,w=width-2*m,h=studio.contentHeight;float headerH=STUDIO_UI_HEADER_H;fill(studio.services.surveillanceTheme.TEXT);surveillanceText(studio.services.surveillanceTheme.FONT_TITLE,true);textAlign(LEFT,CENTER);String title=s.i18n.tr("app.title");fitCurrentTextSize(title,studio.services.surveillanceTheme.FONT_TITLE,10,w,headerH-12);text(ellipsizeToWidth(title,w),m,headerH*0.5f);float top=headerH+g,bottom=64,avail=max(180,h-top-bottom-m-g);if(w>=860){float right=constrain(w*.30f,260,380),left=max(160,w-right-g);drawPreview(m,top,left,avail);drawStatus(m+left+g,top,right,avail);}else{float previewH=max(65,(avail-g)*.55f),statusH=max(55,avail-previewH-g);if(previewH+statusH+g>avail){float k=avail/max(1,previewH+statusH+g);previewH*=k;statusH*=k;}drawPreview(m,top,w,previewH);drawStatus(m,top+previewH+g,w,statusH);}drawButtons(m,h-bottom-m,w,bottom);}
