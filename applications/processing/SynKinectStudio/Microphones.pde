...
void setupMicrophoneModule(){
  ...
  microphoneState().diagnostics=new BridgeDiagnostics(microphoneState().config);microphoneState().captureControl=new AudioCaptureControl();
...
void drawMicrophoneModule(){background(microphoneState().theme.BG);microphoneState().diagnostics.refresh();
 ...
 final HashMap<String,String> values=new HashMap<String,String>();
  ...
  void refreshNow(){
    ...
    if(statusFile==null||!statusFile.isFile()){available=false;readError="missing";
      return;}
    if(now-statusFile.lastModified()>cfg.diagnosticsStaleMs){available=false;readError="stale";
      return;}
  ...
  String stateKey(){
    ...
    if("uac-runtime-capturing".equals(current)){
      if(captureRate()==studio.services.spatialAudioProtocol.SAMPLE_RATE&&captureChannels()>=studio.services.spatialAudioProtocol.CHANNELS&&published()>0)return "diag.ok";
...
class MicrophoneUI {
 ...
 final ArrayList<MicButton> buttons=new ArrayList<MicButton>();
  ...
  void drawTransportPanel(float x,float y,float w,float h){
    ...
    String code=microphoneState().diagnostics.errorCode();if(code.length()>0)reconnects+=" · E"+code;
    ...
    String[] values={transportState,formatCount(microphoneState().source==null?0:microphoneState().source.frameCount()),doa,microphoneState().diagnostics.formatSummary(),
      reconnects,volume};boolean[] active={live,live,spatial!=null,"diag.ok".equals(microphoneState().diagnostics.stateKey()),microphoneState().diagnostics.lastError()==0,
  ...
  String groupStatus(int[] actions){
    ...
    return microphoneState().i18n.tr(microphoneState().diagnostics.stateKey())+" · "+microphoneState().diagnostics.compactCounters();
