...
void setupMicrophoneModule(){
  ...
  studio.microphoneState.pipeline=new AudioPipeline(studio.microphoneState.config,studio.microphoneState.spatialConfig);studio.microphoneState.diagnostics=new BridgeDiagnostics(studio.microphoneState.config);studio.microphoneState.captureControl=new AudioCaptureControl();studio.microphoneState.source=new MicrophoneSource(studio.microphoneState.config,studio.microphoneState.pipeline);studio.microphoneState.ui=new MicrophoneUI();
}

void drawMicrophoneModule(){background(studio.services.microphoneTheme.BG);studio.microphoneState.diagnostics.refresh();if(studio.microphoneState.captureControl!=null)studio.microphoneState.captureControl.refreshIfDue();studio.microphoneState.ui.draw(studio.microphoneState.source==null?null:studio.microphoneState.source.snapshot());}
...
void microphoneKeyPressed(){
  ...
  else if(key=='-'||key=='_')adjustCaptureVolume(-5);
  else if(key=='+'||key=='=')adjustCaptureVolume(5);
  else if(key=='0')toggleCaptureMute();
...
void dispatchMicrophoneAction(int action){
  ...
  else if(action==studio.microphoneState.ui.ACTION_VOL_DOWN)adjustCaptureVolume(-5);
  else if(action==studio.microphoneState.ui.ACTION_VOL_UP)adjustCaptureVolume(5);
  else if(action==studio.microphoneState.ui.ACTION_MUTE)toggleCaptureMute();
}

void adjustCaptureVolume(int delta){if(studio.microphoneState.captureControl!=null)studio.microphoneState.captureControl.setVolume(constrain(studio.microphoneState.captureControl.volumePercent+delta,0,100));}
void toggleCaptureMute(){if(studio.microphoneState.captureControl!=null)studio.microphoneState.captureControl.setMute(!studio.microphoneState.captureControl.muted);}
...
class BridgeDiagnostics {
 ...
 final HashMap<String,String> values=new HashMap<String,String>();
  ...
  String compactCounters(){String backend=studio.services.transportFactory.isLinux()?"ALSA":"WASAPI";return backend+" "+transportPackets()+" · PCM "+published()+" · SESS "+runtimeSessions();}
...
class MicrophoneTheme {
  ...
  final int MARGIN=STUDIO_UI_MARGIN, GAP=STUDIO_UI_GAP, HEADER_H=STUDIO_UI_HEADER_H, STATUS_H=112, CONTROLS_H=122, RADIUS=STUDIO_UI_RADIUS;
  final int FONT_TINY=STUDIO_FONT_TINY, FONT_SMALL=STUDIO_FONT_SMALL, FONT_BODY=STUDIO_FONT_BODY, FONT_LABEL=STUDIO_FONT_LABEL, FONT_METRIC=STUDIO_FONT_METRIC, FONT_TITLE=STUDIO_FONT_TITLE;
}
...
class MicrophoneProtocol {
  ...
  final int CONTROL_MAGIC=0x434D4D52;
  final int CONTROL_GET_STATE=1,CONTROL_SET_VOLUME=2,CONTROL_SET_MUTE=3;
  final int CONTROL_REQUEST_BYTES=80,CONTROL_REPLY_BYTES=32;
}

class AudioControlOp { final int command,value; AudioControlOp(int c,int v){command=c;value=v;} }
class AudioCaptureControl {
  volatile int volumePercent=100;
  volatile boolean muted=false,available=false;
  volatile String backend="—",detail="";
  volatile long lastRefreshMs=0;
  final Object queueLock=new Object();
  final ArrayDeque<AudioControlOp> pending=new ArrayDeque<AudioControlOp>();
  boolean workerRunning=false;
  void refreshIfDue(){
    long now=System.currentTimeMillis();
    synchronized(queueLock){if(workerRunning||!pending.isEmpty()||now-lastRefreshMs<750)return;lastRefreshMs=now;}
    queue(studio.services.microphoneProtocol.CONTROL_GET_STATE,0,false);
  }
  void setVolume(int percent){volumePercent=constrain(percent,0,100);queue(studio.services.microphoneProtocol.CONTROL_SET_VOLUME,volumePercent*100,true);}
  void setMute(boolean value){muted=value;queue(studio.services.microphoneProtocol.CONTROL_SET_MUTE,value?1:0,true);}
  String summary(){return available?(muted?studio.microphoneState.i18n.tr("audio.muted"):(volumePercent+"%"))+" · "+backend:studio.microphoneState.i18n.tr("audio.unavailable");}
  void queue(final int command,final int value,boolean userInitiated){
    boolean launch=false;
    synchronized(queueLock){
      // Background status reads may be skipped. User volume/mute commands are
      // always queued and are executed in order, so rapid button presses cannot
      // race one another or be overwritten by an older request.
      if(!userInitiated&&(workerRunning||!pending.isEmpty()))return;
      pending.addLast(new AudioControlOp(command,value));
      if(!workerRunning){workerRunning=true;launch=true;}
    }
    if(launch)studio.services.workers.startLowPriority("Audio-Control",new Runnable(){public void run(){drain();}});
  }
  void drain(){
    while(true){
      AudioControlOp op;
      synchronized(queueLock){
        op=pending.pollFirst();
        if(op==null){workerRunning=false;return;}
      }
      transact(op.command,op.value);
    }
  }
  void transact(int command,int value){
    LocalTransport pipe=null;
    try{
      TransportEndpoint ep=studio.services.endpoints.audioControl;pipe=studio.services.transportFactory.open(ep.windowsPath,ep.linuxPath);
      ByteBuffer q=ByteBuffer.allocate(studio.services.microphoneProtocol.CONTROL_REQUEST_BYTES).order(ByteOrder.LITTLE_ENDIAN);q.putInt(studio.services.microphoneProtocol.CONTROL_MAGIC).putInt(studio.services.microphoneProtocol.VERSION).putInt(command).putInt(value);putFixedDeviceId(q,selectedKinectDeviceId());pipe.write(q.array());
      byte[] rb=new byte[studio.services.microphoneProtocol.CONTROL_REPLY_BYTES];pipe.readFully(rb);ByteBuffer r=ByteBuffer.wrap(rb).order(ByteOrder.LITTLE_ENDIAN);
      int magic=r.getInt(),version=r.getInt(),result=r.getInt(),basis=r.getInt(),mute=r.getInt(),caps=r.getInt(),backendCode=r.getInt();r.getInt();
      if(magic!=studio.services.microphoneProtocol.CONTROL_MAGIC||version!=studio.services.microphoneProtocol.VERSION||result!=0)throw new IOException("audio-control-"+result);
      volumePercent=constrain(round(basis/100.0f),0,100);muted=mute!=0;available=true;detail="";backend=backendCode==2?"ALSA mixer":backendCode==3?"Windows endpoint":"bridge";
    }catch(Exception e){available=false;detail=safeStudioMessage(e);}
    finally{if(pipe!=null)try{pipe.close();}catch(IOException ignored){}}
  }
}
...
class MicrophoneSource {
 ...
 final Object pipeLock=new Object();
  ...
  void subscribe(LocalTransport pipe)throws IOException{
    ...
    request.putInt(studio.services.microphoneProtocol.MAGIC);request.putInt(studio.services.microphoneProtocol.VERSION);request.putInt(studio.services.microphoneProtocol.CMD_SUBSCRIBE);request.putInt(0);putFixedDeviceId(request,selectedKinectDeviceId());pipe.write(request.array());
...
class MicrophoneUI {
 ...
 final ArrayList<MicButton> buttons=new ArrayList<MicButton>();
  ...
  void drawTransportPanel(float x,float y,float w,float h){
    ...
    String volume=studio.microphoneState.captureControl==null?"—":studio.microphoneState.captureControl.summary();String[] labels={studio.microphoneState.i18n.tr("label.transport"),studio.microphoneState.i18n.tr("label.frames"),studio.i18n.tr("label.doa_beam"),studio.microphoneState.i18n.tr("label.runtime"),studio.microphoneState.i18n.tr("label.reconnects"),studio.microphoneState.i18n.tr("label.volume")};
    String[] values={transportState,formatCount(studio.microphoneState.source==null?0:studio.microphoneState.source.frameCount),doa,studio.microphoneState.diagnostics.formatSummary(),reconnects,volume};boolean[] active={live,live,spatial!=null,"diag.ok".equals(studio.microphoneState.diagnostics.stateKey()),studio.microphoneState.diagnostics.lastError()==0,studio.microphoneState.captureControl!=null&&studio.microphoneState.captureControl.available};
    int cols=w<850?3:6,rows=(labels.length+cols-1)/cols;float top=y+32,gap=8,tw=(w-24-gap*(cols-1))/cols,th=(h-42-gap*(rows-1))/rows;for(int i=0;i<labels.length;i++){int col=i%cols,row=i/cols;metricTile(x+12+col*(tw+gap),top+row*(th+gap),tw,th,labels[i],values[i],active[i]);}
  }
  ...
  void drawControls(float x,float y,float w,float h){
    ...
    drawActionGroup(x+captureW+gap,y,playW,h,studio.microphoneState.i18n.tr("panel.playback"),new int[]{ACTION_PLAY,ACTION_TEST,ACTION_VOL_DOWN,ACTION_VOL_UP,ACTION_MUTE});
  }
  ...
  void drawActionGroup(float x,float y,float w,float h,String title,int[] actions){
    ...
    float bx=x+9,by=y+31,gap=7,bh=STUDIO_UI_BUTTON_H,bw=(w-18-gap*(actions.length-1))/actions.length;
...
class MicButton {
  ...
  boolean active(){
    ...
    if(action==studio.microphoneState.ui.ACTION_MUTE)return studio.microphoneState.captureControl!=null&&studio.microphoneState.captureControl.muted;
