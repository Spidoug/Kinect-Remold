...
class AcousticTheme {
  ...
  final int MARGIN=STUDIO_UI_MARGIN,GAP=STUDIO_UI_GAP,HEADER_H=STUDIO_UI_HEADER_H,RADIUS=STUDIO_UI_RADIUS,CARD_TITLE_H=STUDIO_UI_CARD_TITLE_H;
...
class AcousticSource {
 ...
 final Object frameLock=new Object(),pipeLock=new Object();
  ...
  void subscribe(LocalTransport pipe)throws IOException{
    ByteBuffer q=ByteBuffer.allocate(studio.services.acousticProtocol.REQUEST_BYTES).order(ByteOrder.LITTLE_ENDIAN);q.putInt(studio.services.acousticProtocol.MAGIC);q.putInt(studio.services.acousticProtocol.VERSION);q.putInt(studio.services.acousticProtocol.CMD_SUBSCRIBE);q.putInt(0);putFixedDeviceId(q,selectedKinectDeviceId());pipe.write(q.array());
...
class AcousticUI {
  float resetX,resetY,resetW=112,buttonH=STUDIO_UI_BUTTON_H,autoX,manualX,modeW=112;
