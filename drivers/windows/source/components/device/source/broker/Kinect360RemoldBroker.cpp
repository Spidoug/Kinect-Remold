...
std::wstring FirstDeviceLocationPath(HDEVINFO info, SP_DEVINFO_DATA& dev) {
    ...
    (void)SetupDiGetDeviceRegistryPropertyW(info, &dev, SPDRP_LOCATION_PATHS, &regType, nullptr, 0, &needed);
    if (needed < sizeof(wchar_t)) return {};
    std::vector<BYTE> buffer(needed + sizeof(wchar_t), 0);
    if (!SetupDiGetDeviceRegistryPropertyW(info, &dev, SPDRP_LOCATION_PATHS, &regType, buffer.data(), needed, nullptr)) return {};
...
std::string PhysicalDeviceId(HDEVINFO set, SP_DEVINFO_DATA& dev, const wchar_t* fallbackPath) {
    std::wstring identity = FirstDeviceLocationPath(set, dev);
    if (!identity.empty()) identity = SensorLocationRoot(std::move(identity));
    if (identity.empty()) {
        wchar_t instance[512]{};
        if (SetupDiGetDeviceInstanceIdW(set, &dev, instance, static_cast<DWORD>(std::size(instance)), nullptr) && instance[0]) identity = instance;
        else if (fallbackPath) identity = fallbackPath;
    }
    if (identity.empty()) return {};
    std::wostringstream token;
    token << L"winusb-" << std::hex << std::setfill(L'0') << std::setw(16) << StableDeviceHash(identity);
    return WideToUtf8(token.str());
}

HRESULT OpenPhysicalInterface(const GUID& guid, PhysicalTransportKind kind, PhysicalNuiSession& session, const std::string& targetDeviceId) {
    ...
    for (DWORD index = 0;; ++index) {
        ...
        const std::string deviceId = PhysicalDeviceId(set, dev, detail->DevicePath);
        if (!targetDeviceId.empty() && deviceId != targetDeviceId) continue;
...
HRESULT OpenPhysicalNui(PhysicalNuiSession& session, const std::string& targetDeviceId) {
    ...
    HRESULT hr = OpenPhysicalInterface(Hardware::kNuiTransportGuid, PhysicalTransportKind::Classic1414, session, targetDeviceId);
    if (SUCCEEDED(hr)) return hr;
    session.Close();
    return OpenPhysicalInterface(Hardware::kAudioControlTransportGuid, PhysicalTransportKind::AudioControl1473, session, targetDeviceId);
}

HRESULT EnsurePhysicalNui(const std::string& targetDeviceId) {
    if (gPhysicalNuiSession.Ready() && (targetDeviceId.empty() || gPhysicalNuiSession.deviceId == targetDeviceId)) return S_OK;
    gPhysicalNuiSession.Close();
    return OpenPhysicalNui(gPhysicalNuiSession, targetDeviceId);
}
...
// fresh reopen through the common retry interface.
...
HRESULT SetLed1473(PhysicalNuiSession& session, LONG value) {
    // Normalize the common LED interface onto the subset exposed by the 1473
...
HRESULT WaitForTiltTarget(PhysicalNuiSession& session, LONG degrees, Reply& observed) {
    constexpr LONG kToleranceTenths = 15; // same 1.5-degree success interface for both revisions
...
HRESULT TryPhysicalNuiCommand(const Request& request, Reply& reply) {
    ...
    // backend gets the same one-fresh-handle retry interface. Backend-specific
    // USB recovery remains local to the connection implementation.
    std::lock_guard<std::mutex> physicalGuard(gPhysicalNuiMutex);
    const std::string targetDeviceId(request.deviceId, strnlen(request.deviceId, sizeof(request.deviceId)));

    auto execute = [&](Reply& out) -> HRESULT {
        const HRESULT openHr = EnsurePhysicalNui(targetDeviceId);
    ...
    // One fresh-handle retry is part of the common interface. Each backend may
...
namespace Nui = Kinect360RemoldNui;
namespace Sdk = Kinect360RemoldNui::Sdk;

constexpr uint32_t kNuiCapabilities =
    Nui::CapabilityColor | Nui::CapabilityInfrared | Nui::CapabilityDepth |
    Nui::CapabilityAudio4Mic | Nui::CapabilityTiltLedAccel |
    Nui::CapabilityNui20JointModel | Nui::CapabilityRawSensorFrames |
    Nui::CapabilitySkeletonStream | Nui::CapabilityAudioControl | Nui::CapabilitySdkBridge;

void CopyEndpoint(char* destination, size_t capacity, const char* value) {
    if (!destination || capacity == 0) return;
    std::memset(destination, 0, capacity);
    if (!value) return;
    const size_t valueLength = std::strlen(value);
    const size_t copyLength = (valueLength < (capacity - 1)) ? valueLength : (capacity - 1);
    if (copyLength != 0) std::memcpy(destination, value, copyLength);
}

struct ManifestDevice {
    std::string id;
    std::string label;
    std::string cameraEndpoint;
};
...
std::vector<ManifestDevice> ReadDeviceManifest() {
    std::vector<ManifestDevice> devices;
    ...
    while (std::getline(in, line)) {
        ...
        ManifestDevice item{line.substr(0, a), line.substr(a + 1, b - a - 1), line.substr(b + 1)};
        if (!item.id.empty() && !item.cameraEndpoint.empty()) devices.push_back(std::move(item));
    }
    std::sort(devices.begin(), devices.end(), [](const ManifestDevice& a, const ManifestDevice& b) { return a.id < b.id; });
    devices.erase(std::unique(devices.begin(), devices.end(), [](const ManifestDevice& a, const ManifestDevice& b) { return a.id == b.id; }), devices.end());
    return devices;
}

void CopyDeviceId(Request& request, const std::string& deviceId) {
    const size_t n = std::min(deviceId.size(), sizeof(request.deviceId) - 1);
    if (n) std::memcpy(request.deviceId, deviceId.data(), n);
}

bool SensorPresent(const std::string& deviceId, Reply* observed = nullptr) {
    Request ping{};
    ping.command = Command::Ping;
    CopyDeviceId(ping, deviceId);
    Reply state{};
    const HRESULT hr = TryPhysicalNuiCommand(ping, state);
    if (observed) *observed = state;
    return SUCCEEDED(hr);
}

void FillNuiEndpoints(Nui::Reply& reply) {
    CopyEndpoint(reply.cameraEndpoint, sizeof(reply.cameraEndpoint), "\\\\.\\pipe\\Kinect360RemoldScanner-*");
    CopyEndpoint(reply.audioEndpoint, sizeof(reply.audioEndpoint), "\\\\.\\pipe\\Kinect360RemoldAudio");
    CopyEndpoint(reply.controlEndpoint, sizeof(reply.controlEndpoint), "\\\\.\\pipe\\Kinect360RemoldControl");
    CopyEndpoint(reply.skeletonEndpoint, sizeof(reply.skeletonEndpoint), "\\\\.\\pipe\\Kinect360RemoldNuiSkeleton");
    CopyEndpoint(reply.audioControlEndpoint, sizeof(reply.audioControlEndpoint), "\\\\.\\pipe\\Kinect360RemoldAudioControl");
    CopyEndpoint(reply.sdkEndpoint, sizeof(reply.sdkEndpoint), "\\\\.\\pipe\\Kinect360RemoldSdk");
}

void FillSdkEndpoints(Sdk::Reply& reply, const ManifestDevice* device) {
    if (device) CopyEndpoint(reply.cameraEndpoint, sizeof(reply.cameraEndpoint), device->cameraEndpoint.c_str());
    CopyEndpoint(reply.audioEndpoint, sizeof(reply.audioEndpoint), "\\\\.\\pipe\\Kinect360RemoldAudio");
    CopyEndpoint(reply.audioControlEndpoint, sizeof(reply.audioControlEndpoint), "\\\\.\\pipe\\Kinect360RemoldAudioControl");
    CopyEndpoint(reply.skeletonEndpoint, sizeof(reply.skeletonEndpoint), "\\\\.\\pipe\\Kinect360RemoldNuiSkeleton");
}

HRESULT DispatchNuiRequest(const Nui::Request& request, Nui::Reply& reply) {
    reply = {};
    FillNuiEndpoints(reply);
    if (request.magic != Nui::kMagic || request.version != Nui::kVersion || request.command != Nui::Command::RuntimeInfo)
        return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);
    return ReadDeviceManifest().empty() ? HRESULT_FROM_WIN32(ERROR_DEVICE_NOT_CONNECTED) : S_OK;
}

HRESULT DispatchSdkRequest(const Sdk::Request& request, Sdk::Reply& reply) {
    reply = {};
    reply.capabilities = kNuiCapabilities;
    if (request.magic != Sdk::kMagic || request.version != Sdk::kVersion)
        return HRESULT_FROM_WIN32(ERROR_INVALID_DATA);

    const auto sensors = ReadDeviceManifest();
    reply.sensorCount = static_cast<uint32_t>(sensors.size());
    if (request.command == Sdk::Command::SensorCount) {
        FillSdkEndpoints(reply, sensors.empty() ? nullptr : &sensors.front());
        reply.status = sensors.empty() ? 0u : 1u;
        return S_OK;
    }

    const std::string requested(request.deviceId, strnlen(request.deviceId, sizeof(request.deviceId)));
    const ManifestDevice* sensor = nullptr;
    if (!requested.empty()) {
        for (const auto& item : sensors) if (item.id == requested) { sensor = &item; break; }
    } else if (request.sensorIndex < sensors.size()) {
        sensor = &sensors[request.sensorIndex];
    }
    FillSdkEndpoints(reply, sensor);
    if (!sensor) return HRESULT_FROM_WIN32(ERROR_DEVICE_NOT_CONNECTED);
    reply.status = 1;

    auto queryStatus = [&]() -> HRESULT {
        Request controlRequest{};
        controlRequest.command = Command::Status;
        CopyDeviceId(controlRequest, sensor->id);
        Reply state{};
        const HRESULT hr = TryPhysicalNuiCommand(controlRequest, state);
        if (SUCCEEDED(hr)) {
            reply.elevationDegrees = state.tiltTenths / 10;
            reply.accelX = state.accelX;
            reply.accelY = state.accelY;
            reply.accelZ = state.accelZ;
        }
        return hr;
    };
    ...
    switch (request.command) {
        ...
        case Sdk::Command::Initialize:
            reply.initializedFlags = request.flags;
            return S_OK;
        case Sdk::Command::Shutdown:
            reply.initializedFlags = 0;
            return S_OK;
        case Sdk::Command::SetElevation: {
            if (request.value < Policy::kTiltMinDegrees || request.value > Policy::kTiltMaxDegrees) return E_INVALIDARG;
            Request controlRequest{};
            controlRequest.command = Command::Tilt;
            controlRequest.value = request.value;
            CopyDeviceId(controlRequest, sensor->id);
            Reply state{};
            const HRESULT hr = TryPhysicalNuiCommand(controlRequest, state);
            if (SUCCEEDED(hr)) {
...
PSECURITY_DESCRIPTOR MakeLocalPipeDescriptor() {
...
template <typename TRequest, typename TReply, typename TDispatch>
void RequestReplyPipeLoop(const wchar_t* pipeName, std::atomic<bool>* active, TDispatch dispatch) {
    PSECURITY_DESCRIPTOR descriptor = MakeLocalPipeDescriptor();
    ...
    while (active->load() && gRun.load()) {
        HANDLE pipe = CreateNamedPipeW(
            ...
            8, sizeof(TReply), sizeof(TRequest), 1000, descriptor ? &security : nullptr);
        ...
        if (connected) {
            ...
            if (ReadFile(pipe, &request, sizeof(request), &read, nullptr) && read == sizeof(request)) {
                TReply reply{};
                const HRESULT hr = dispatch(request, reply);
...
void NuiServerLoop(std::atomic<bool>* active) {
    RequestReplyPipeLoop<Nui::Request, Nui::Reply>(Nui::kPipeName, active, DispatchNuiRequest);
}
void SdkServerLoop(std::atomic<bool>* active) {
    RequestReplyPipeLoop<Sdk::Request, Sdk::Reply>(Nui::kSdkPipeName, active, DispatchSdkRequest);
}

void WakeNamedPipe(const wchar_t* pipeName) {
    if (!WaitNamedPipeW(pipeName, 100)) return;
    HANDLE pipe = CreateFileW(pipeName, GENERIC_READ | GENERIC_WRITE, 0, nullptr, OPEN_EXISTING,
                              FILE_ATTRIBUTE_NORMAL, nullptr);
    if (pipe != INVALID_HANDLE_VALUE) CloseHandle(pipe);
}

class SkeletonRelayServer {
public:
    ~SkeletonRelayServer() { Stop(); }
    bool Start(std::atomic<bool>* active) {
        active_ = active;
    ...
    void Stop() {
        ...
        WakeNamedPipe(Nui::kSkeletonPipeName);
        {
            std::lock_guard<std::mutex> guard(mu_);
            for (auto& item : publishers_) {
                (void)CancelIoEx(item.second, nullptr);
                (void)DisconnectNamedPipe(item.second);
            }
            for (auto& sub : subscribers_) {
                (void)DisconnectNamedPipe(sub.pipe);
                CloseHandle(sub.pipe);
            }
            subscribers_.clear();
        }
        if (thread_.joinable()) thread_.join();
        for (auto& publisherThread : publisherThreads_) if (publisherThread.joinable()) publisherThread.join();
        std::lock_guard<std::mutex> guard(mu_);
        for (auto& item : publishers_) CloseHandle(item.second);
        publishers_.clear();
    }
private:
    struct Subscriber { HANDLE pipe = INVALID_HANDLE_VALUE; std::string deviceId; };
    std::atomic<bool>* active_ = nullptr;
    std::atomic<bool> stopping_{false};
    std::mutex mu_;
    std::vector<Subscriber> subscribers_;
    std::map<std::string, HANDLE> publishers_;
    std::thread thread_;
    std::vector<std::thread> publisherThreads_;

    std::string ResolveDeviceId(const char* raw, size_t capacity) {
        std::string id(raw, strnlen(raw, capacity));
        if (!id.empty()) return id;
        const auto devices = ReadDeviceManifest();
        return devices.empty() ? std::string{} : devices.front().id;
    }
    bool SensorExists(const std::string& id) {
        if (id.empty()) return false;
        const auto devices = ReadDeviceManifest();
        return std::any_of(devices.begin(), devices.end(), [&](const ManifestDevice& device) { return device.id == id; });
    }
    HANDLE CreateInstance() {
        ...
        security.lpSecurityDescriptor = descriptor;
        HANDLE pipe = CreateNamedPipeW(
            Nui::kSkeletonPipeName, PIPE_ACCESS_DUPLEX,
            PIPE_TYPE_MESSAGE | PIPE_READMODE_MESSAGE | PIPE_WAIT | PIPE_REJECT_REMOTE_CLIENTS,
            32, 64 * 1024, 64 * 1024, 1000, descriptor ? &security : nullptr);
        if (descriptor) LocalFree(descriptor);
        return pipe;
    }
    void AcceptLoop() {
        while (!stopping_.load() && active_ && active_->load() && gRun.load()) {
            HANDLE pipe = CreateInstance();
            if (pipe == INVALID_HANDLE_VALUE) { Sleep(200); continue; }
            BOOL connected = ConnectNamedPipe(pipe, nullptr);
            if (!connected && GetLastError() == ERROR_PIPE_CONNECTED) connected = TRUE;
            if (!connected) { CloseHandle(pipe); continue; }
            Nui::SkeletonHello hello{};
            DWORD read = 0;
            if (!ReadFile(pipe, &hello, sizeof(hello), &read, nullptr) || read != sizeof(hello) ||
                hello.magic != Nui::kMagic || hello.version != Nui::kSkeletonVersion) {
                DisconnectNamedPipe(pipe); CloseHandle(pipe); continue;
            }
            Nui::SkeletonReply reply{};
            const std::string deviceId = ResolveDeviceId(hello.deviceId, sizeof(hello.deviceId));
            if (!SensorExists(deviceId)) {
                reply.result = static_cast<int32_t>(HRESULT_FROM_WIN32(ERROR_DEVICE_NOT_CONNECTED));
                DWORD written = 0; (void)WriteFile(pipe, &reply, sizeof(reply), &written, nullptr);
                DisconnectNamedPipe(pipe); CloseHandle(pipe); continue;
            }
            if (hello.role == Nui::SkeletonRole::Subscribe) {
                DWORD written = 0;
                if (!WriteFile(pipe, &reply, sizeof(reply), &written, nullptr) || written != sizeof(reply)) {
                    DisconnectNamedPipe(pipe); CloseHandle(pipe); continue;
                }
                DWORD pipeMode = PIPE_READMODE_MESSAGE | PIPE_NOWAIT;
                (void)SetNamedPipeHandleState(pipe, &pipeMode, nullptr, nullptr);
                std::lock_guard<std::mutex> guard(mu_);
                subscribers_.push_back(Subscriber{pipe, deviceId});
                continue;
            }
            if (hello.role == Nui::SkeletonRole::Publish) {
                ...
                {
                    std::lock_guard<std::mutex> guard(mu_);
                    busy = publishers_.find(deviceId) != publishers_.end();
                    if (!busy) publishers_[deviceId] = pipe;
                }
                if (busy) {
                ...
                DWORD written = 0;
                if (!WriteFile(pipe, &reply, sizeof(reply), &written, nullptr) || written != sizeof(reply)) {
                    std::lock_guard<std::mutex> guard(mu_); publishers_.erase(deviceId);
                    DisconnectNamedPipe(pipe); CloseHandle(pipe); continue;
                }
                publisherThreads_.emplace_back([this, pipe, deviceId] { PublisherLoop(pipe, deviceId); });
                continue;
            }
    ...
    void PublisherLoop(HANDLE pipe, const std::string& deviceId) {
        Nui::SkeletonFrame frame{};
        while (!stopping_.load() && active_ && active_->load() && gRun.load()) {
            DWORD available = 0;
            if (!PeekNamedPipe(pipe, nullptr, 0, nullptr, &available, nullptr)) break;
            if (available < sizeof(frame)) { Sleep(4); continue; }
            DWORD read = 0;
            if (!ReadFile(pipe, &frame, sizeof(frame), &read, nullptr) || read != sizeof(frame)) break;
            if (frame.magic != Nui::kSkeletonFrameMagic || frame.version != Nui::kSkeletonVersion ||
                frame.bodyCount > Nui::kMaxSkeletons) continue;
            Broadcast(deviceId, frame);
        }
        bool ownHandle = false;
        {
            std::lock_guard<std::mutex> guard(mu_);
            auto it = publishers_.find(deviceId);
            if (it != publishers_.end() && it->second == pipe) { publishers_.erase(it); ownHandle = true; }
        }
        if (ownHandle) { DisconnectNamedPipe(pipe); CloseHandle(pipe); }
    }
    void Broadcast(const std::string& deviceId, const Nui::SkeletonFrame& frame) {
        std::lock_guard<std::mutex> guard(mu_);
        for (auto it = subscribers_.begin(); it != subscribers_.end();) {
            if (it->deviceId != deviceId) { ++it; continue; }
            DWORD written = 0;
            if (!WriteFile(it->pipe, &frame, sizeof(frame), &written, nullptr) || written != sizeof(frame)) {
                DisconnectNamedPipe(it->pipe); CloseHandle(it->pipe); it = subscribers_.erase(it);
            } else {
                ++it;
            }
        }
...
int RunHost() {
    ...
    std::thread nuiThread(NuiServerLoop, &brokerRun);
    std::thread sdkThread(SdkServerLoop, &brokerRun);
    SkeletonRelayServer skeletonRelay;
    (void)skeletonRelay.Start(&brokerRun);
    while (gRun.load()) Sleep(250);
    brokerRun.store(false);
    WakeControlServer();
    WakeNamedPipe(Nui::kPipeName);
    WakeNamedPipe(Nui::kSdkPipeName);
    skeletonRelay.Stop();
    if (controlThread.joinable()) controlThread.join();
    if (nuiThread.joinable()) nuiThread.join();
