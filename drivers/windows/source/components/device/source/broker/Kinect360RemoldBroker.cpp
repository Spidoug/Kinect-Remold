...
HRESULT OpenPhysicalNui(PhysicalNuiSession& session, const std::string& targetId) {
    ...
    // pinned to 02BB/02C3&MI_00. An unscoped request, or a sensor
...
HRESULT Prepare1414Control(PhysicalNuiSession& session) {
    if (session.kind != PhysicalTransportKind::Classic1414) return S_OK;
    if (session.prepared) return S_OK;
    const HRESULT hr = SetLed1414(session, static_cast<LONG>(Kinect360RemoldControl::LedMode::Green));
    if (SUCCEEDED(hr)) session.prepared = true;
    return hr;
}
...
HRESULT SetLed1473(PhysicalNuiSession& session, LONG value) {
    const bool wasPrepared = session.prepared;
    const HRESULT prepareHr = Prepare1473Control(session);
    if (FAILED(prepareHr)) return prepareHr;

    int32_t alt = 0;
    if (value == static_cast<LONG>(Kinect360RemoldControl::LedMode::Off)) alt = 1;
    else if (value == static_cast<LONG>(Kinect360RemoldControl::LedMode::Green)) alt = 3;
    ...
    if (alt == 3 && !wasPrepared) return S_OK;
    return SendAltCommand(session, Hardware::AudioControl::kLedCommand, alt);
}
...
HRESULT SetPhysicalLed(PhysicalNuiSession& session, LONG value) {
    ...
    const PhysicalControlOps* ops = ControlOpsFor(session.kind);
    if (!ops) return E_HANDLE;
    const HRESULT hr = ops->setLed(session, value);
    if (SUCCEEDED(hr)) session.prepared = true;
    return hr;
}
