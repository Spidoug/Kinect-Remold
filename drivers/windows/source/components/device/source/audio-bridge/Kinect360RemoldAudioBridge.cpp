...
bool TryResolveCommonDataDirectory(std::wstring& directory) {
    ...
    directory += L"\\";
    directory += AudioPort::kStatusDirectory;
    return true;
}

std::string NarrowForStatus(const std::wstring& value) {
    std::string result;
...
public:
    ...
    void SetEndpointName(const std::wstring& value) {
        ...
        m_endpointName = NarrowForStatus(value);
    }

    void SetEndpointBinding(const std::wstring& instanceId, const std::string& deviceId) {
        std::lock_guard<std::mutex> guard(m_lock);
        m_endpointInstanceId = NarrowForStatus(instanceId);
    ...
    void Write(bool force = false) {
        ...
        const std::wstring path = directory + L"\\" + AudioPort::kStatusFileName;
...
std::unique_ptr<UsbSession> OpenBootAudioUsb(const std::set<std::wstring>& suppressedKeys = {}) {
    ++gStatus.usbOpenAttempts;
    if (!Usb::Ready()) {
        gStatus.SetStage("winusb-load-error", Usb::Get().LoadError(), "system winusb.dll is unavailable");
        return {};
    }
    auto& api = Usb::Get();
...
HRESULT FindKinectUacCaptureEndpoints(std::vector<UacEndpointCandidate>& endpoints) {
    ++gStatus.wasapiEndpointSearches;
    endpoints.clear();
...
public:
    ...
    void Push(const BYTE* data, UINT32 frames, DWORD flags, const WAVEFORMATEX* format, SampleEncoding encoding) {
        ...
        if (silent) gStatus.wasapiSilentFrames.fetch_add(frames);
        if (flags & AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY) ++gStatus.wasapiDiscontinuities;
...
HRESULT RunWasapiCaptureSession(IMMDevice* endpoint,
    ...
    gStatus.SetEndpointName(friendlyName);
    gStatus.SetCaptureMode(captureMode);
    gStatus.captureRate = selected->nSamplesPerSec;
    gStatus.captureChannels = selected->nChannels;
    gStatus.captureBits = selected->wBitsPerSample;
    gStatus.captureFormatTag = selected->wFormatTag;
    ++gStatus.runtimeSessions;
    gStatus.SetStage("uac-runtime-capturing", ERROR_SUCCESS, captureMode);
    ...
    while (gRun.load() && (!nodeRun || nodeRun->load(std::memory_order_acquire))) {
        ...
        if (wait == WAIT_TIMEOUT) { gStatus.Write(); continue; }
...
HRESULT CaptureUacRuntime(IMMDevice* endpoint, const std::wstring& friendlyName, RawAudioBus& port, const std::atomic<bool>* nodeRun = nullptr) {
    ...
    gStatus.SetEndpointName(friendlyName);
    gStatus.SetCaptureMode("probing-shared");
    ...
    if (!IsUsableCaptureFormat(mixFormat)) {
        ...
        gStatus.SetCaptureMode("unsupported-shared-format");
    ...
    CoTaskMemFree(mixFormat);
    if (FAILED(capture)) ++gStatus.wasapiOpenFailures;
    return capture;
}
...
void FirmwareLoop() {
    ...
    while (gRun.load()) {
        ...
        if (boot) {
            ...
            ++gStatus.bootSessions;
            gStatus.SetStage("uac-firmware-uploading", ERROR_SUCCESS,
            ...
            boot.reset();
            if (SUCCEEDED(upload)) {
                ++gStatus.firmwareUploads;
                if (!key.empty()) {
                    launchedKeys.insert(key);
                    absentPasses[key] = 0;
                }
                gStatus.SetStage("uac-firmware-launched", ERROR_SUCCESS,
                ...
                ++gStatus.firmwareFailures;
                gStatus.SetStage("uac-firmware-error", HRESULT_CODE(upload),
        ...
        while (gRun.load() && m_run.load(std::memory_order_acquire)) {
            ...
            if (SUCCEEDED(FindKinectUacCaptureEndpoints(endpoints))) {
                ...
                for (auto& endpoint : endpoints) {
                    ...
                    attempted = true;
                    gStatus.SetEndpointBinding(endpoint.instanceId, endpoint.deviceId);
                ...
                if (!attempted) {
                    gStatus.SetStage("uac-runtime-reconnecting", ERROR_NOT_READY,
