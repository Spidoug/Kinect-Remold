...
$Work=Join-Path (Get-RemoldExternalWorkRoot) 'driver\device'
$Dist=Join-Path $Work 'stage'
$Cache=Join-Path (Get-RemoldExternalCacheRoot) 'driver\device'
...
try{
    ...
    Write-Host ('Raw mic output : {0}<device-id> -> dedicated Processing 4-channel monitor' -f $Product.AudioPipePrefix)
    Write-Host ''
    ...
    $brokerOut=Join-Path $Work 'native\broker\out';$brokerInt=Join-Path $Work 'native\broker\obj';New-Item -ItemType Directory -Force $brokerOut,$brokerInt|Out-Null
    $brokerBuildArgs=@(
        $brokerProject,'/m','/t:Rebuild',"/p:Configuration=$Configuration",'/p:Platform=x64',
        "/p:RemoldPlatformToolset=$($Tools.PlatformToolset)","/p:PlatformToolset=$($Tools.PlatformToolset)",
        "/p:WindowsTargetPlatformVersion=$($Tools.WdkVersion)","/p:RemoldGeneratedInclude=$DevicePolicyWork",
        "/p:OutDir=$brokerOut\","/p:IntDir=$brokerInt\"
    )
    & $Tools.MSBuild @brokerBuildArgs
    if($LASTEXITCODE){throw 'Kinect360RemoldBroker build failed.'}
    $broker=Get-ChildItem -LiteralPath $brokerOut -Recurse -File -Filter Kinect360RemoldBroker.exe|Sort-Object LastWriteTime -Descending|Select-Object -First 1
    ...
    $nuiOut=Join-Path $Work 'native\nui\out';$nuiInt=Join-Path $Work 'native\nui\obj';New-Item -ItemType Directory -Force $nuiOut,$nuiInt|Out-Null
    $nuiProject=Join-Path $Root 'source\nui\Kinect360RemoldNui.vcxproj'
    $nuiBuildArgs=@(
        $nuiProject,'/m','/t:Rebuild',"/p:Configuration=$Configuration",'/p:Platform=x64',
        "/p:RemoldPlatformToolset=$($Tools.PlatformToolset)","/p:PlatformToolset=$($Tools.PlatformToolset)",
        "/p:WindowsTargetPlatformVersion=$($Tools.WdkVersion)","/p:OutDir=$nuiOut\","/p:IntDir=$nuiInt\"
    )
    & $Tools.MSBuild @nuiBuildArgs
    if($LASTEXITCODE){throw 'Kinect360RemoldNui build failed.'}
    $nuiTool=Get-ChildItem -LiteralPath $nuiOut -Recurse -File -Filter Kinect360RemoldNui.exe|Sort-Object LastWriteTime -Descending|Select-Object -First 1
    ...
    if($expectedFirmwareVersion -ne '01.02.709.00'){throw ("Unsupported Kinect UACFirmware version: {0}" -f $expectedFirmwareVersion)}
    if(Test-Path -LiteralPath $localFirmwarePath -PathType Leaf){
        ...
        if(!(Test-Path -LiteralPath $runtimeBundle -PathType Leaf)){
            Write-BuildStage 'Downloading Microsoft Kinect for Windows Runtime v1.8 package...'
    ...
    # UACFirmware 01.02.709.00 builds its USB descriptors in ARM code at runtime.
    # The host cannot override bInterval, so use the configured Microsoft firmware image and
    # validate only the audio format contract consumed by the Remold bridges.
    ...
    if($targetEndpoint -ne 0x82 -or $targetAttributes -ne 0x0D -or $targetPacket -ne 524 -or
       $targetRate -ne 16000 -or $targetChannels -ne 4 -or $targetBits -ne 32){
        throw ("Unsupported Kinect UAC audio contract: ep=0x{0:X2} attr=0x{1:X2} packet={2} rate={3} channels={4} bits={5}" -f $targetEndpoint,$targetAttributes,$targetPacket,$targetRate,$targetChannels,$targetBits)
    }
    ...
    Write-Host ("UACFirmware {0} validation: PASS (SHA256={1}, bytes={2}, load=0x00080000, entry=0x00080030; audio=EP0x82/ISO-SYNC/524/4ch/16k/S32; Windows capture=shared mode; models=1414,1473)" -f $expectedFirmwareVersion,$firmwareSha,$fw.Length) -ForegroundColor Green
    ...
    $audioOut=Join-Path $Work 'native\audio-bridge\out';$audioInt=Join-Path $Work 'native\audio-bridge\obj';New-Item -ItemType Directory -Force $audioOut,$audioInt|Out-Null
    $audioBuildArgs=@(
        $audioProject,'/m','/t:Rebuild',"/p:Configuration=$Configuration",'/p:Platform=x64',
        "/p:RemoldPlatformToolset=$($Tools.PlatformToolset)","/p:PlatformToolset=$($Tools.PlatformToolset)",
        "/p:WindowsTargetPlatformVersion=$($Tools.WdkVersion)","/p:RemoldGeneratedInclude=$FirmwareWork",
        "/p:OutDir=$audioOut\","/p:IntDir=$audioInt\"
    )
    & $Tools.MSBuild @audioBuildArgs
    if($LASTEXITCODE){throw 'Kinect360RemoldAudioBridge build failed.'}
    $audioBridge=Get-ChildItem -LiteralPath $audioOut -Recurse -File -Filter Kinect360RemoldAudioBridge.exe|Sort-Object LastWriteTime -Descending|Select-Object -First 1
