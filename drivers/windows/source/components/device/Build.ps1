...
try{
    ...
    # The host cannot override bInterval, so pin the Microsoft firmware image and
    # validate only the audio format interface consumed by the Remold bridges.
    ...
    if($targetEndpoint -ne 0x82 -or $targetAttributes -ne 0x0D -or $targetPacket -ne 524 -or
       $targetRate -ne 16000 -or $targetChannels -ne 4 -or $targetBits -ne 32){
        throw ("Unsupported Kinect V1 UAC audio interface: ep=0x{0:X2} attr=0x{1:X2} packet={2} rate={3} channels={4} bits={5}" -f $targetEndpoint,$targetAttributes,$targetPacket,$targetRate,$targetChannels,$targetBits)
    }
