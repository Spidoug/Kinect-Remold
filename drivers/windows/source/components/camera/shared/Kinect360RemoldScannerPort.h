...
// Private local scanner transport. One pipe is exposed per physical Kinect.
...
enum DriverSetting : uint32_t {
    DriverSettingRgbHighQuality = 0x00000001u,
    DriverSettingSupported = DriverSettingRgbHighQuality,
};
...
// ScannerPort is sensor-native only. Any consumer that needs display or
