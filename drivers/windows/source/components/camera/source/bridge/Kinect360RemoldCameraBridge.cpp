...
bool BrokerExchange(Control::Command command, int32_t value, const std::string& deviceId, Control::Reply& reply, DWORD waitMs = 100) {
    reply = {};
    ...
    request.value = value;
    const size_t idBytes = std::min(deviceId.size(), sizeof(request.deviceId) - 1);
    if (idBytes) std::memcpy(request.deviceId, deviceId.data(), idBytes);
...
bool QueryBrokerMotion(const std::string& deviceId, ScannerPort::MotionSample& motion) {
    motion = {};
    Control::Reply reply{};
    if (!BrokerExchange(Control::Command::Status, 0, deviceId, reply, 0) ||
...
bool PrepareCameraControl(const std::string& deviceId) {
    Control::Reply reply{};
    return BrokerExchange(Control::Command::PrepareCamera, 0, deviceId, reply, 1000);
}


bool GlobalRgbHqEnabled() {
    DWORD value = 0;
    DWORD bytes = sizeof(value);
    const LSTATUS status = RegGetValueW(HKEY_LOCAL_MACHINE, L"SOFTWARE\\Kinect360Remold",
                                        L"RgbHqEnabled", RRF_RT_REG_DWORD, nullptr, &value, &bytes);
...
public:
    ...
    void SubmitVga(const uint8_t* nv12, size_t bytes, uint64_t tickMs) {
        ...
        PublishFrame(nv12, bytes, tickMs, 0u, kWidth, kHeight);
    }

    void SubmitHq(const uint8_t* nv12, size_t bytes, uint64_t tickMs) {
        if (!nv12 || bytes != kHqNv12Bytes) return;
        PublishFrame(nv12, bytes, tickMs, 1u, kHqWidth, kHqHeight);
    }
...
private:
    ...
    void PublishFrame(const uint8_t* nv12, size_t bytes, uint64_t tickMs, uint32_t flags, uint32_t width, uint32_t height) {
        if (!m_header || !nv12) return;
        if (!((width == kWidth && height == kHeight && bytes == kNv12Bytes) ||
              (width == kHqWidth && height == kHqHeight && bytes == kHqNv12Bytes))) return;
        InterlockedIncrement(&m_header->colorSequence);
...
public:
    explicit ScannerPortServer(std::wstring pipeName, std::string deviceId) : m_pipeName(std::move(pipeName)), m_deviceId(std::move(deviceId)) {}
        ...
        for (;;) {
            ...
            switch (m_mode) {
                ...
                case ScannerPort::StreamMode::RgbHighQuality: {
                    ...
                    // NV12 for the Windows camera interface. If Scanner also needs the
...
public:
    ...
    bool Push(const uint8_t* packet, ULONG length, std::vector<uint8_t>& completed,
        ...
        if (h.flag == sof) {
            ...
            // remains independent of conversion-cost differences.
...
std::wstring SensorLocationRoot(std::wstring location) {
    // Kinect functions are siblings below the sensor's internal USB hub. Drop
    // only the final USB child segment so camera, control and audio resolve to
    // the same physical sensor while separate Kinects remain distinct.
...
public:
    ...
    HRESULT Open(const std::wstring& devicePath, const std::string& deviceId) {
        Close();
...
private:
    ...
    HRESULT StreamVideo(SharedFramePublisher& publisher,
        ...
        Log(L"Kinect video endpoint 0x81 active.");
        while (SUCCEEDED(result) && SessionRunning()) {
            const ULONGLONG now = GetTickCount64();
            const bool hqRequested = GlobalRgbHqEnabled() || scannerPort.Wants(ScannerPort::StreamMode::RgbHighQuality);
            ...
            if (activeMode == ScannerPort::StreamMode::RgbHighQuality && completeFramesInMode == 0 &&
                ...
                Log(L"RGB-HQ produced no complete 1280x1024 frame; using standard RGB for this session.");
                hqSuppressed = true;
            }
    ...
    HRESULT StreamDepthSession(ScannerPortServer& scannerPort, FrameConversionWorker& depthWorker) {
        ...
        // keeps endpoint 0x82 registered across normal close/reopen cycles.
        ...
        // cancelling/unregistering endpoint 0x82. This prevents a host-side ISO
        // teardown from leaving the Kinect firmware latched in its Depth session.
...
public:
    CameraNode(CameraInterfaceInfo info, SharedFramePublisher& primaryPublisher, bool publishesVirtualCamera)
        : m_info(std::move(info)), m_primaryPublisher(primaryPublisher), m_publishVirtual(publishesVirtualCamera),
          m_scannerPort(ScannerPipeName(m_info.id), m_info.id) {}
...
private:
    ...
    void Loop() {
        while (g_run.load() && m_run.load(std::memory_order_acquire)) {
            ...
            HRESULT hr = camera.Open(m_info.path, m_info.id);
            if (FAILED(hr)) {
                wchar_t message[256]{};
            ...
            // Scanner motion has one logical interface on both revisions. The
