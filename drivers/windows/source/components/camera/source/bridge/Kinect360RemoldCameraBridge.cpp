...
// Mirrors remold::MotionSampler on Linux: polling runs only while the camera
// session is online and a frame consumer is active, uses the per-model
// cadence, and Current() never returns a sample older than the model's stale
...
public:
    ...
    HRESULT Open(const std::wstring& devicePath, const std::string& deviceId, bool startupOpen) {
        ...
        // runtime while RGB/IR/Depth remain on the independent 02AE camera
        // function. Give the control sibling time to appear at startup, but do
        // not add that enumeration delay to later image-stream recovery.
        if (m_model == Hardware::Model::Xbox1473) {
            ...
            if (!prepared && startupOpen)
                Log(L"Kinect 1473 control keep-alive is unavailable; continuing with the independent camera transport.");
        }
