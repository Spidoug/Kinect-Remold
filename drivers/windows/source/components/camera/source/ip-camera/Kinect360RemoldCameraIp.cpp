...
void Nv12ToBgr(const uint8_t* nv12, uint32_t sourceWidth, uint32_t sourceHeight, std::vector<uint8_t>& bgr) {
    const int width = static_cast<int>(sourceWidth);
    const int height = static_cast<int>(sourceHeight);
...
bool EncodeJpegWic(IWICImagingFactory* factory, const std::vector<uint8_t>& bgr, uint32_t width, uint32_t height, uint32_t quality, std::vector<uint8_t>& jpeg) {
    if (!factory || !width || !height || bgr.size() != static_cast<size_t>(width) * height * 3u) return false;
    ...
    if (SUCCEEDED(factory->CreateEncoder(GUID_ContainerFormatJpeg, nullptr, &encoder)) &&
        ...
        if (SUCCEEDED(frame->Initialize(props)) &&
            ...
            SUCCEEDED(frame->WritePixels(height, stride, static_cast<UINT>(bgr.size()), const_cast<BYTE*>(bgr.data()))) &&
...
public:
    ...
    bool Read(std::vector<uint8_t>& nv12, uint32_t& width, uint32_t& height, uint64_t& frameNumber, uint64_t& tickMs, bool& online) {
...
void EncoderLoop() {
    ...
    uint32_t frameWidth = Frame::kWidth, frameHeight = Frame::kHeight;
    ...
    while (g_running.load()) {
        ...
        while (g_running.load()) {
            ...
            if (reader.Read(nv12, frameWidth, frameHeight, frameNumber, tickMs, online)) {
                ...
                if (online && frameNumber != 0 && frameNumber != lastFrame && now >= nextEncode) {
                    ...
                    Nv12ToBgr(nv12.data(), frameWidth, frameHeight, bgr);
                    jpeg.clear();
                    if (EncodeJpegWic(factory, bgr, frameWidth, frameHeight, g_config.jpegQuality, jpeg)) {
