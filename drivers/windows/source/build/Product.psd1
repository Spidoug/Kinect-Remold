@{
    ...
    # Smart Tilt control law (shared controller, Kinect360RemoldSmartTilt.h),
    # mirrored by the smart_tilt.* keys of the Linux remold.conf. Motion comes
    # from the per-frame sample of CameraBridge; Smart Tilt never polls status.
    SmartTiltPolicy = @{
        FacePeriodMs = 40
        CommandPeriodMs = 90
    ...
    # Native IP-camera runtime. It follows one configured/Ready Kinect through the
    # per-device RGB shared-memory transport and never opens the Kinect USB device itself.
    CameraIpPolicy = @{
        # Secure-by-default. The installer creates a strong credential but does
        # not expose the camera on the network until the user explicitly opts in.
        Enabled = $false
        Bind = '127.0.0.1'
        LanBind = '0.0.0.0'
        Port = 8088
    ...
    WindowsCameraNamePrefix = 'Kinect Xbox 360 Camera ['
    AudioPipePrefix = '\\.\pipe\Kinect360RemoldAudio-'
    ...
    # Kinect Runtime microphone data contract used by the application bridge.
    # The 1473 keeps its separate MI_00 control topology; MI_02 remains owned by
    # Windows USB Audio and its descriptor timing is firmware-defined.
