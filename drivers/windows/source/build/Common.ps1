...
function Get-RemoldExternalCacheRoot{
    if(![string]::IsNullOrWhiteSpace($env:REMOLD_CACHE_ROOT)){return [IO.Path]::GetFullPath($env:REMOLD_CACHE_ROOT)}
    $base=$env:LOCALAPPDATA
    if([string]::IsNullOrWhiteSpace($base)){$base=[Environment]::GetFolderPath([Environment+SpecialFolder]::LocalApplicationData)}
    if([string]::IsNullOrWhiteSpace($base)){$base=[IO.Path]::GetTempPath()}
    return [IO.Path]::GetFullPath((Join-Path $base 'Kinect360Remold\Cache\windows'))
}

function Get-RemoldExternalWorkRoot{
    if(![string]::IsNullOrWhiteSpace($env:REMOLD_WORK_ROOT)){return [IO.Path]::GetFullPath($env:REMOLD_WORK_ROOT)}
    $base=$env:LOCALAPPDATA
    if([string]::IsNullOrWhiteSpace($base)){$base=[Environment]::GetFolderPath([Environment+SpecialFolder]::LocalApplicationData)}
    if([string]::IsNullOrWhiteSpace($base)){$base=[IO.Path]::GetTempPath()}
    return [IO.Path]::GetFullPath((Join-Path $base 'Kinect360Remold\Work\windows'))
}

function Resolve-PinnedDependencyUrl([string]$Template,[string]$Commit,[string]$Label){
    if([string]::IsNullOrWhiteSpace($Template)){throw "Dependency URL template is empty: $Label"}
    if($Commit -notmatch '^[0-9a-fA-F]{40}$'){throw "Dependency commit is invalid while resolving URL: $Label"}
    $url=($Template -f $Commit)
    $uri=$null
    if(![Uri]::TryCreate($url,[UriKind]::Absolute,[ref]$uri) -or $uri.Scheme -ne 'https'){throw "Dependency URL must be absolute HTTPS ($Label): $url"}
    return $url
}
