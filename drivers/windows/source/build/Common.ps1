...
function Get-MsBuildNamespaceManager([System.Xml.XmlDocument]$Xml){
    ...
    # therefore part of this helper's interface and must not be removed.
    return ,$ns
}
...
function Assert-WdkProjectInterface(
    [string]$Path,
    ...
    if(![string]::IsNullOrWhiteSpace($RequiredInf)){
        ...
        if($infNodes.Count -ne 1){throw "WDK project interface missing INF item '$RequiredInf': $Path"}
    }
    $platformLabel=if([string]::IsNullOrWhiteSpace($expectedDriverTarget)){'supported'}else{$expectedDriverTarget}
    Write-Host ("WDK project interface: PASS ({0}, configurations={1}, type={2}, platform={3}, toolset=WindowsKernelModeDriver10.0)" -f (Split-Path -Leaf $Path),$projectConfigs.Count,$ConfigurationType,$platformLabel) -ForegroundColor Green
}
