...
if([string]::IsNullOrWhiteSpace($script:WinGet)){
    throw 'WinGet was not found. Windows 11 normally provides it through Microsoft App Installer. Install/update App Installer from Microsoft Store and run Kinect-Xbox-360-Remold.cmd --rebuild again.'
}

if(!(Test-Path -LiteralPath $OfficialConfig -PathType Leaf)){
    throw "Official WDK WinGet configuration is missing: $OfficialConfig"
}
if(!(Test-Path -LiteralPath $VsConfig -PathType Leaf)){
    throw "Official Visual Studio component configuration is missing: $VsConfig"
}
...
) 'Applying Microsoft WDK development configuration' -AllowFailure)
...
throw ('Automatic Microsoft toolchain bootstrap did not produce a usable MSBuild/SDK/WDK environment. ' +
       'Review the WinGet/Visual Studio output above. If Windows requested a restart, restart once and run Kinect-Xbox-360-Remold.cmd --rebuild again; all successfully installed components will be reused.')
