...
function Invoke-SelfAction([string]$RequestedAction){
    & $PSCommandPath -Action $RequestedAction -DeviceId $DeviceId
}
function Get-NuiArguments([object[]]$Arguments){
    if([string]::IsNullOrWhiteSpace($DeviceId)){return @($Arguments)}
    return @('--device',$DeviceId)+@($Arguments)
}
function Need-Tools{
...
function Show-Status{
    ...
    Show-State 'Virtual camera' $cameraReady '640x480/30 output; RGB HQ improves its source when enabled'
    Show-State 'Scanner transport' $cameraBridgeReady 'RGB/RGB HQ + metric Depth transport'
    ...
    if(![string]::IsNullOrWhiteSpace($DeviceId)){
        Write-Host ''
        Write-Host ("Selected Kinect: {0}" -f $DeviceId) -ForegroundColor Cyan
        [void](Invoke-NativeCode $Nui (Get-NuiArguments @('status')) -Show)
    }
    Show-RgbHq
}
...
function Get-RgbHqEnabled{
    try{
        $value=Get-ItemPropertyValue -Path 'HKLM:\SOFTWARE\Kinect360Remold' -Name 'RgbHqEnabled' -ErrorAction Stop
        return ([int]$value -ne 0)
    }catch{return $false}
}
function Show-RgbHq{
    $enabled=Get-RgbHqEnabled
    Write-Host ("RGB HQ: {0}" -f $(if($enabled){'ENABLED'}else{'DISABLED'})) -ForegroundColor $(if($enabled){'Green'}else{'DarkGray'})
}
function Toggle-RgbHq{
    Require-Administrator
    $path='HKLM:\SOFTWARE\Kinect360Remold'
    if(!(Test-Path -LiteralPath $path)){[void](New-Item -Path $path -Force)}
    $next=if(Get-RgbHqEnabled){0}else{1}
    New-ItemProperty -Path $path -Name 'RgbHqEnabled' -PropertyType DWord -Value $next -Force | Out-Null
    Show-RgbHq
}
...
function Set-Tilt{
    ...
    $code=Invoke-NativeCode $Nui (Get-NuiArguments @('tilt',[string]$value)) -Show
...
if($Action -eq 'Install' -and (!(Test-Path -LiteralPath $Nui -PathType Leaf) -or !(Test-Path -LiteralPath $Setup -PathType Leaf))){
    ...
    if(Test-Path -LiteralPath $sourceBuild -PathType Leaf){
        Header
        Write-Host 'Native driver/runtime is not built yet. Building the v1.0 distribution first...' -ForegroundColor Cyan
...
$PrivilegedActions=@('Install','IpReset','IpToggle','RgbHqToggle','Uninstall')
...
if($Action -ne 'Menu'){
    ...
    switch($Action){
        ...
        'Tilt'{Set-Tilt}
        'StartupTilt'{if(Need-Tools){$code=Invoke-NativeCode $Nui (Get-NuiArguments @('tilt',[string]$Product.StartupTiltDegrees)) -Show;if($code -eq 0){Write-Host ("Tilt returned to the {0:+#;-#;0} degree startup pose." -f $Product.StartupTiltDegrees) -ForegroundColor Green}}}
        'IpStatus'{Show-CameraIp}
        'IpReset'{Reset-CameraIpPassword}
        'IpToggle'{Toggle-CameraIp}
        'RgbHqToggle'{Toggle-RgbHq}
...
while($true){
    ...
    Write-Host '9  Enable / Disable RGB HQ'
    ...
    switch($choice){
        ...
        '5'{Header;if(Need-Tools){$code=Invoke-NativeCode $Nui (Get-NuiArguments @('tilt',[string]$Product.StartupTiltDegrees)) -Show;if($code -eq 0){Write-Host ("Tilt returned to the {0:+#;-#;0} degree startup pose." -f $Product.StartupTiltDegrees) -ForegroundColor Green}};Pause-Menu}
        ...
        '9'{Header;Invoke-SelfAction 'RgbHqToggle';Pause-Menu}
        '10'{Header;Open-Studio;Start-Sleep -Milliseconds 500}
        '11'{Header;Invoke-SelfAction 'Uninstall';Pause-Menu}
        '0'{return}
