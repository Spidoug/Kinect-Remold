[CmdletBinding()]
param(
    [ValidateSet('Menu','Install','Status','OpenCamera','Tilt','StartupTilt','RgbHqStatus','RgbHqOn','RgbHqOff','RgbHqToggle','IpStatus','IpCredentials','IpReset','IpLocal','IpLan','IpDevice','IpToggle','OpenStudio','Restart','Diagnose','Uninstall')]
    [string]$Action='Menu',
    [string]$DeviceId='',
    [switch]$NoPause
)
...
function Get-KinectFunctionName([string]$ProductId,[string]$Revision){
    switch($ProductId){
        '02AE'{if($Revision -eq '010B'){return 'Xbox 360 model 1414 camera'}else{return 'Xbox 360 model 1473 camera'}}
        '02B0'{return '1414 dedicated motor/control'}
        '02C2'{return '1473 parent USB hub/controller (must not be claimed/reset)'}
        '02AD'{return 'Kinect audio bootloader (UAC firmware pending)'}
        {$_ -in '02BB','02C3'}{return 'Kinect UAC runtime composite (MI_00 control on 1473; MI_02 audio)'}
    }
    return 'other Microsoft USB function'
}
function Show-Diagnostics{
    if(!(Need-Tools)){return}
    Write-Section 'Kinect Xbox 360 Remold - Windows diagnostics'
    Write-Host ("date={0}" -f (Get-Date -Format o))
    Write-Host ("os={0}" -f [Environment]::OSVersion.VersionString)
    Write-Host ("installed_version={0}" -f $Product.Version)

    Write-Section 'USB model/topology'
    $usbFunctions=@(Get-PnpDevice -PresentOnly -ErrorAction SilentlyContinue|Where-Object{$_.InstanceId -match '^USB\\VID_045E&PID_(02AE|02B0|02C2|02AD|02BB|02C3)(&|\\)'})
    foreach($usbFunction in $usbFunctions){
        $productId=([regex]::Match($usbFunction.InstanceId,'PID_([0-9A-F]{4})')).Groups[1].Value
        $hardwareIds=@((Get-PnpDeviceProperty -InstanceId $usbFunction.InstanceId -KeyName 'DEVPKEY_Device_HardwareIds' -ErrorAction SilentlyContinue).Data)
        $revision=''
        foreach($hardwareId in $hardwareIds){$match=[regex]::Match([string]$hardwareId,'REV_([0-9A-F]{4})');if($match.Success){$revision=$match.Groups[1].Value;break}}
        Write-Host ("{0,-56} rev={1,-4} status={2,-8} service={3,-14} {4}" -f $usbFunction.InstanceId,$(if($revision){$revision}else{'????'}),$usbFunction.Status,$usbFunction.Service,(Get-KinectFunctionName $productId $revision))
    }
    if(!$usbFunctions.Count){Write-Host 'No supported Kinect 1414/1473 USB functions are present.'}

    Write-Section 'Remold runtime'
    foreach($key in @($Product.ServiceOrder)){
        $name=$Product.Services[$key]
        $service=Get-Service $name -ErrorAction SilentlyContinue
        Write-Host ("{0,-34} {1}" -f $name,$(if($null -ne $service){[string]$service.Status}else{'not installed'}))
    }
    foreach($file in @('devices.tsv','audio-bridge-status.txt')){
        $path=Join-Path $env:ProgramData ('Kinect360Remold\'+$file)
        if(Test-Path -LiteralPath $path -PathType Leaf){Write-Host '';Write-Host ('--- '+$path+' ---');Get-Content -LiteralPath $path}
    }

    Write-Section 'Camera self-test (ScannerPort, same path as Studio and the virtual camera)'
    $rows=@(Get-DeviceManifest)
    foreach($row in $rows){
        Write-Host ''
        Write-Host ("{0} ({1}) state={2}" -f $row.Label,$row.Id,$row.State)
        if([string]::IsNullOrWhiteSpace([string]$row.Camera)){Write-Host '  camera endpoint not published';continue}
        [void](Invoke-NativeCode $Nui @('--device',$row.Id,'probe','rgb+depth','5') -Show)
        [void](Invoke-NativeCode $Nui @('--device',$row.Id,'probe','ir','4') -Show)
    }
    if(!$rows.Count){Write-Host 'devices.tsv is not published.'}

    Write-Section 'Virtual cameras'
    foreach($row in $rows){
        [void](Invoke-NativeCode $WebcamCtl @('status',$row.Id) -Show)
        [void](Invoke-NativeCode $Nui @('--device',$row.Id,'frames','3') -Show)
    }

    Write-Section 'Recent service logs'
    foreach($log in @(Get-ChildItem -LiteralPath (Join-Path $env:ProgramData 'Kinect360Remold') -Filter '*.log' -File -ErrorAction SilentlyContinue)){
        Write-Host ''
...
while($true){
    ...
    Write-Host '14 Open SynKinect Studio (Windows)'
    Write-Host '15 Restart Kinect runtime (administrator)'
    Write-Host '16 Diagnose 1414 / 1473 USB, audio and services'
    Write-Host '17 Uninstall'
    Write-Host '0  Exit'
    Write-Host ''
    ...
    switch($choice){
        ...
        '16'{Header;Show-Diagnostics;Pause-Menu}
        '17'{Header;Invoke-SelfAction 'Uninstall';Pause-Menu}
        '0'{return}
