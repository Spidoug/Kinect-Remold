...
if($Action -eq 'Install' -and (!(Test-Path -LiteralPath $Nui -PathType Leaf) -or !(Test-Path -LiteralPath $Setup -PathType Leaf))){
    ...
    $builtControl=if([string]::IsNullOrWhiteSpace($repoRoot)){$null}else{Join-Path $repoRoot 'binaries\windows\drivers\system\Kinect.ps1'}
    if(Test-Path -LiteralPath $sourceBuild -PathType Leaf){
        ...
        if([string]::IsNullOrWhiteSpace($builtControl) -or !(Test-Path -LiteralPath $builtControl -PathType Leaf)){Write-Host 'Build completed but the generated Kinect control script was not found.' -ForegroundColor Red;Pause-Menu;return}
