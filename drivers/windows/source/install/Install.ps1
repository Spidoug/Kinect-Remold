...
function Stabilize-AudioFunction{
    ...
    # observations. If diagnostics show an active firmware transition, wait for the
...
try{
    ...
    if($script:State.Audio -eq 'PENDING'){
        ...
        Write-Host ("Audio diagnostics       : stage={0}; last_error={1}" -f $stage,$error) -ForegroundColor DarkYellow
        Write-Host ("Audio diagnostics file  : {0}" -f (Join-Path $env:ProgramData 'Kinect360Remold\audio-bridge-status.txt')) -ForegroundColor DarkGray
    }
