...
function Invoke-SetupInteractive([string[]]$Arguments=@()){
    ...
    # terminating NativeCommandError while preserving the installer's explicit exit-code handling
...
try{
    ...
    $rgbPolicyKey='HKLM:\SOFTWARE\Kinect360Remold'
    if(!(Test-Path -LiteralPath $rgbPolicyKey)){[void](New-Item -Path $rgbPolicyKey -Force)}
    if($null -eq (Get-ItemProperty -Path $rgbPolicyKey -Name 'RgbHqEnabled' -ErrorAction SilentlyContinue)){New-ItemProperty -Path $rgbPolicyKey -Name 'RgbHqEnabled' -PropertyType DWord -Value 0 -Force|Out-Null}
