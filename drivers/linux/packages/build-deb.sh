...
# Debian packages use the same Linux source build and generated distribution as the normal driver build.
REMOLD_BUILD_DIR="$BUILD" REMOLD_DIST_DIR="$STAGE/usr" \
  bash "$SRC/scripts/build.sh" --clean --no-deps
...
install -m 0644 "$ROOT/../../docs/linux/PLATFORM-RUNTIME.md" "$PKG/usr/share/doc/kinect360-remold/PLATFORM-RUNTIME.md"
...
Description: Kinect Xbox 360 Remold native Linux runtime
 ...
 V4L2 virtual camera and authenticated IP-camera runtime for Kinect 1414/1473.
CONTROL
...
case "${1:-configure}" in
  configure)
    ...
    /usr/libexec/kinect360-remold/ensure-v4l2-device.sh >/dev/null 2>&1 || true
    KINECT_PRESENT=0
...
udevadm control --reload-rules >/dev/null 2>&1 || true
if [ "${1:-}" = purge ]; then
  rm -rf /var/lib/kinect360-remold
fi
exit 0
POSTRM
