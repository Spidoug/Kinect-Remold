...
if [[ -d "$HERE/libexec/kinect360-remold" || -x "$HERE/bin/kinect360-remoldctl" ]]; then BUNDLE="$HERE";else PROJECT="$(cd "$HERE/../.." && pwd)";BUNDLE="$PROJECT/binaries/linux/drivers";fi
...
# Refuse a runtime that was built from other driver sources than the project
# it belongs to; installing it would silently keep old broker/camera/audio code.
PROJECT_ROOT="$(cd "$BUNDLE/../../.." && pwd)"
if [[ -f "$PROJECT_ROOT/drivers/linux/FINGERPRINT.sh" ]]; then
  if [[ "$(bash "$PROJECT_ROOT/drivers/linux/FINGERPRINT.sh")" != "$(cat "$BUNDLE/BUILD-FINGERPRINT" 2>/dev/null)" ]]; then
    echo 'The runtime in binaries/linux/drivers was not built from the current driver sources.' >&2
    echo 'Rebuild it first: bash Kinect-Xbox-360-Remold.sh --build-only' >&2
    exit 2
  fi
fi
# Installation is deliberately offline. Every package download, firmware fetch
# and kernel-module compilation belongs to the BUILD stage. No install-time
# compatibility switches are accepted: a built bundle is the only input.
...
REQUIRED_BUNDLE_FILES=(
  ...
  "$BUNDLE/DIAGNOSE.sh"
  "$BUNDLE/VERSION"
  "$BUNDLE/BUILD-FINGERPRINT"
  "$BUNDLE/libexec/kinect360-remold/kinect360-remold-sdk"
  ...
  "$SUPPORT/systemd/kinect360-remold-sdk.service"
...
RUNTIME_UNITS=(
  ...
  kinect360-remold-sdk.service
  kinect360-remold-camera-ip.service
)

stop_installed_runtime(){
  # Reinstall is a clean runtime replacement. Product configuration is kept;
  # no replaced executable, unit or socket stays alive while the bundle is
  # installed.
...
install -D -m 0755 "$BUNDLE/DIAGNOSE.sh" /usr/share/kinect360-remold/DIAGNOSE.sh
install -D -m 0644 "$BUNDLE/VERSION" /usr/share/kinect360-remold/VERSION
install -D -m 0644 "$BUNDLE/BUILD-FINGERPRINT" /usr/share/kinect360-remold/BUILD-FINGERPRINT
...
verify_installed_file "$BUNDLE/DIAGNOSE.sh" /usr/share/kinect360-remold/DIAGNOSE.sh
verify_installed_file "$BUNDLE/VERSION" /usr/share/kinect360-remold/VERSION
verify_installed_file "$BUNDLE/BUILD-FINGERPRINT" /usr/share/kinect360-remold/BUILD-FINGERPRINT
...
# Wait for the service graph and local product bus to settle. If a Kinect is
# connected, wait a little longer for the 02AD -> 02BB/02C3 audio transition so
# model 1473 control and microphones are ready before setup returns.
...
for _ in $(seq 1 80); do
  if systemctl is-active --quiet kinect360-remold-broker.service \
      ...
      && systemctl is-active --quiet kinect360-remold-sdk.service \
...
for service in broker camera audio sdk v4l2; do
...
verify_running_executable kinect360-remold-sdk.service /usr/libexec/kinect360-remold/kinect360-remold-sdk
...
printf 'Kinect Xbox 360 Remold %s installed (build %s).\n' "$(tr -d '\r\n' <"$BUNDLE/VERSION")" "$(cut -c1-12 "$BUNDLE/BUILD-FINGERPRINT")"
