...
# Installation is deliberately offline. Build/download work belongs to
# Kinect-Xbox-360-Remold.sh --build-only, never to this control panel.
...
# Startup pose: tilt.startup from remold.conf, product default +6 degrees
# (Windows Product.psd1 StartupTiltDegrees).
...
ip_toggle_root(){
  ...
  if [[ "$next" == true ]]; then
    ...
    printf 'IP camera enabled. Current bind mode is preserved; review option 6 before exposing it.\n'
  else
...
run_diagnostics(){
  local candidates=(
    "$ROOT/DIAGNOSE.sh"
    "$ROOT/../../drivers/linux/DIAGNOSE.sh"
    "/usr/share/kinect360-remold/DIAGNOSE.sh"
  )
  local script
  for script in "${candidates[@]}"; do
    if [[ -f "$script" ]]; then bash "$script"; return $?; fi
  done
  printf 'DIAGNOSE.sh was not found in this runtime.\n' >&2
  return 1
}
...
run_action(){
  case "$1" in
    ...
    Diagnose) header; run_diagnostics ;;
...
while true; do
  ...
  printf '%s\n' \
    ...
    '14 Open SynKinect Studio (Linux)' \
    '15 Restart Kinect runtime (administrator)' \
    '16 Diagnose 1414 / 1473 USB, audio and services' \
    '17 Uninstall' \
    '0  Exit'
  printf '\nChoose: '
  ...
  case "${choice//[[:space:]]/}" in
    ...
    16) run_action Diagnose || true; pause_menu ;;
    17) run_action Uninstall || true; pause_menu ;;
    0) exit 0 ;;
