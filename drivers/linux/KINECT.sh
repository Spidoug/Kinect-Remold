...
find_project_root(){
  ...
  for _ in {1..16}; do
    if [[ -f "$dir/VERSION" \
       && -f "$dir/Kinect-Xbox-360-Remold.sh" \
       && -f "$dir/scripts/linux/BUILD-DRIVER.sh" \
       && -d "$dir/drivers/linux" ]]; then
      printf '%s\n' "$dir"
      return 0
    fi
    parent="$(dirname "$dir")"
...
prepare_install_bundle(){
  ...
  project="$(find_project_root)" || {
    printf 'Could not locate the Kinect Xbox 360 Remold project root.\n' >&2
  ...
  if ! bundle_ready "$bundle"; then
    launcher="$project/Kinect-Xbox-360-Remold.sh"
...
install_driver(){
  ...
  printf 'Using prepared runtime: %s\n' "$bundle"
  printf 'Administrator permission is required only for the system installation step.\n'
  run_root bash "$bundle/INSTALL.sh"
}
