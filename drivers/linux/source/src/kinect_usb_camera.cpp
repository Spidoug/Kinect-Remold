...
class Camera::Impl {
 public:
  ...
  int open(const DeviceInfo& wanted, RgbCallback rgb_cb, HqRgbCallback hq_rgb_cb, IrCallback ir_cb, DepthCallback depth_cb) {
    ...
    for (ssize_t i = 0; i < count; ++i) {
      ...
      if (desc.idVendor != kMicrosoftVid || desc.idProduct != kNuiCameraPid) continue;
    ...
    zero_plane_reply_bytes_ = (selected_desc.bcdDevice != 267) ? 334 : 322;
  ...
  void close() {
    if (handle_ != nullptr) {
      // Final physical-session teardown. Normal module/stream close only stops
      // the Kinect firmware engine; the ISO rings stay allocated so a second
      // open does not churn endpoint 0x81/0x82. Free them only here.
      if (video_running_) (void)write_register(0x05, 0x00);
      if (projector_known_ && projector_on_) (void)write_register(0x06, 0x00);
  ...
  int start_video(VideoMode mode) {
    ...
    if (video_running_ && video_mode_ == mode) return 0;

    // Match the Windows V1 lifetime: endpoint 0x81 owns one ISO ring for the
    // whole physical camera session. Mode handoff stops firmware first, waits
    // briefly, cancels/re-arms the same transfers, then configures/starts again.
    if (video_running_) {
      const int stop_rc = write_register(0x05, 0x00);
    ...
    int iso_rc = 0;
    if (!video_stream_.started) {
      iso_rc = start_iso(video_stream_, kVideoEndpoint, kVideoPacketBytes,
      ...
      iso_rc = rearm_iso(video_stream_);
    }
    if (iso_rc < 0) return iso_rc;

    int rc = 0;
    // Exact Windows V1 register sequence. ISO is armed before register 0x05
    // starts the video engine, and ConfigureVideoModeRegisters is represented
    // here without transport-specific calls mixed into the sequence.
    ...
    } else if (mode == VideoMode::RgbHighQuality) {
      if (rc >= 0) rc = write_register(0x105, 0x00);
      if (rc >= 0) rc = write_register(0x13, 0x01);
      if (rc >= 0) rc = write_register(0x14, 0x1e);
      if (rc >= 0) rc = set_projector(true);
      if (rc >= 0) rc = set_projector(false);
      if (rc >= 0) rc = write_register(0x0c, 0x00);
      if (rc >= 0) rc = write_register(0x0d, 0x02);
      if (rc >= 0) rc = write_register(0x0e, 0x0f);
    } else {
    ...
    if (rc >= 0) rc = write_register(0x05, mode == VideoMode::Infrared ? 0x03 : 0x01);
    if (rc >= 0) rc = write_register(mode == VideoMode::Infrared ? 0x48 : 0x47, 0x00);

    if (rc < 0) {
      (void)write_register(0x05, 0x00);
      // A register/transport failure ends this physical session; the outer
      // recovery loop will reopen a clean device rather than reuse a half-armed ring.
      stop_iso(video_stream_);
      return rc;
    }
    video_running_ = true;
  ...
  int stop_video() {
    if (!video_running_) return 0;
    int rc = 0;
    if (handle_ != nullptr) rc = write_register(0x05, 0x00);
    video_running_ = false;
    // Do not free/cancel endpoint 0x81 here. It belongs to the physical camera
    // session and is re-armed by start_video() on the next module/mode open.
    rgb_assembler_.reset();
    rgb_hq_assembler_.reset();
    ir_assembler_.reset();
    return rc;
  ...
  int set_projector(bool on) {
    if (handle_ == nullptr) return LIBUSB_ERROR_NO_DEVICE;
    if (projector_known_ && projector_on_ == on) return 0;
    const int rc = write_register(0x06, on ? 0x02 : 0x00);
    if (rc >= 0) {
      projector_known_ = true;
      projector_on_ = on;
    }
    return rc;
  }
  ...
  int start_depth() {
    ...
    int rc = 0;
    if (!depth_stream_.started) {
      // First depth subscriber: known OFF state -> configure -> arm endpoint
      // 0x82 -> projector ON. Afterwards the ring remains pending for the
      // whole physical session, exactly like the Windows depth worker.
      rc = write_register(0x105, 0x00);
      if (rc >= 0) rc = set_projector(false);
  ...
  int stop_depth() {
    ...
    depth_running_ = false;
    // Keep endpoint 0x82 armed; projector OFF intentionally produces no ISO
    // traffic and the pending transfers wake immediately on the next start.
    depth_assembler_.reset();
    return rc;
  }
  ...
  int reset_device() {
    ...
    video_running_ = false;
    depth_running_ = false;
    // USB reset invalidates outstanding transfers: this is a final local
    // teardown, not a normal module handoff.
    stop_iso(video_stream_);
    stop_iso(depth_stream_);
    ...
    ir_assembler_.reset();
    depth_assembler_.reset();
    if (rc == LIBUSB_ERROR_NOT_FOUND) device_dead_.store(true);
    return rc;
  }
 ...
 private:
  ...
  int start_iso(IsoStream& stream, uint8_t endpoint, int fallback_packet_size,
    ...
    // The Kinect protocol still frames video/depth in fixed logical chunks
    // (1920/1760 bytes including the 12-byte packet header), but libusb ISO
    // transfer slots must be sized to the endpoint's negotiated transaction
    // capacity. This keeps the transfer ring full while
    // the frame assembler below keeps enforcing the Kinect logical payload.
    if (negotiated > 0 && negotiated < fallback_packet_size) return LIBUSB_ERROR_OVERFLOW;
  ...
  int rearm_iso(IsoStream& stream) {
    if (!stream.started || stream.slots.empty()) return LIBUSB_ERROR_NOT_FOUND;
    ...
    for (auto& slot : stream.slots) {
      ...
      if (rc == LIBUSB_ERROR_NOT_FOUND && slot->active) {
        slot->active = false;
        --stream.active_count;
      } else if (rc < 0 && rc != LIBUSB_ERROR_NOT_FOUND) {
        stream.stopping = false;
        return rc;
      }
    }
    ...
    while (ctx_ != nullptr && stream.active_count > 0 && std::chrono::steady_clock::now() < deadline) {
    ...
    stream.stopping = false;
    for (auto& slot : stream.slots) {
      if (slot->transfer == nullptr) return LIBUSB_ERROR_OTHER;
      const int rc = libusb_submit_transfer(slot->transfer);
      if (rc < 0) {
        stream.stopping = true;
        // Let final teardown cancel any slots already re-submitted.
        return rc;
      }
      slot->active = true;
      ++stream.active_count;
  ...
  int send_command(uint16_t command, const uint8_t* payload, std::size_t payload_bytes,
    ...
    int actual = 0;
    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(kControlReplyDeadlineMs);
    for (;;) {
      actual = libusb_control_transfer(handle_, 0xc0, 0, 0, 0, input.data(),
                                       static_cast<uint16_t>(input.size()), 1000);
      if (actual < 0) return actual;
      // Windows accepts the first non-empty short reply. A full-size transfer is
      // treated as not-yet-the-command-reply and polled again until the same 2 s deadline.
...
std::vector<DeviceInfo> enumerate() {
  ...
  if (count >= 0) {
    for (ssize_t i = 0; i < count; ++i) {
      ...
      for (int p = 0; p < sensor_ports; ++p) { if (p) id << '.'; id << static_cast<unsigned>(ports[static_cast<std::size_t>(p)]); }
      info.id = id.str();
...
int Camera::reset_device() { return impl_->reset_device(); }
