...
// Keep sixteen packets in each of sixteen asynchronous transfers so the USB
// queue remains deep enough for continuous isochronous delivery.
 ...
 public:
  ...
  void process(const uint8_t* packet, int length) {
    ...
    if (!synced_) {
      ...
      // Short packet gaps are represented as zero-filled regions so stale pixels
      // are never reused in a published frame.
...
class Camera::Impl {
 public:
  ...
  int open(const DeviceInfo& wanted, RgbCallback rgb_cb, HqRgbCallback hq_rgb_cb, IrCallback ir_cb, DepthCallback depth_cb) {
    ...
    // Discover the video/depth alternate setting from live descriptors.
  ...
  int start_video(VideoMode mode) {
    ...
    // Configure the video engine while idle, arm endpoint 0x81, then start the
    // stream with register 0x05.
  ...
  // Configure the depth path while disabled, arm endpoint 0x82, then enable
  // the projector/depth engine. Reconfiguring the engine after ISO is armed can
  // reset the endpoint session.
  int start_depth() {
    if (handle_ == nullptr) return LIBUSB_ERROR_NO_DEVICE;
 ...
 private:
  ...
  void discover_camera_stream_interface() {
    ...
    bool found = false;
    // Prefer interface 0, then accept any interface whose descriptors expose
    // both camera isochronous endpoints.
  ...
  void stop_iso(IsoStream& stream) {
    ...
    // Pump events until every submitted transfer retires so callbacks cannot
    // reference freed transfer slots during reconnect or shutdown.
  ...
  int send_command(uint16_t command, const uint8_t* payload, std::size_t payload_bytes,
    ...
    for (;;) {
      ...
      // Zero-length and full-buffer reads mean the reply is not ready yet.
  ...
  bool fetch_depth_calibration() {
    ...
    // Fixed-parameter replies vary in total size; only the calibrated fields
    // consumed below are required.
    ...
    // The constant shift is the second 16-bit word; firmware may append data.
...
std::vector<DeviceInfo> enumerate() {
  ...
  if (count >= 0) {
    for (ssize_t i = 0; i < count; ++i) {
      ...
      info.id = id.str();
      // The display label names the model; the stable ID remains a separate field.
