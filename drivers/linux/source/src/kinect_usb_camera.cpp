...
// libfreenect uses 16 packets x 16 transfers on Linux. Keep that proven
// queue geometry here; Windows uses a different WinUSB-specific batching
// strategy (32 packets) because its scheduling API has different constraints.
constexpr int kPacketsPerTransfer = 16;
constexpr int kVideoPacketBytes = 1920;
constexpr int kDepthPacketBytes = 1760;
constexpr int kControlReplyDeadlineMs = 2000;
...
class FrameAssembler {
 public:
  ...
  // New stream session: resynchronize and restart the counters.
  void restart() {
    reset();
    stats_ = {};
  }
  ...
  void process(const uint8_t* packet, int length) {
    ...
    if (!synced_) {
      ...
      // Fixed-stream recovery tolerates a short burst of missing packets and keeps
      // the frame cadence. Zero filling is preferable
    ...
    if (header.flag == eof) {
      if (packet_index_ == packets_per_frame_) {
        ++stats_.frames;
        if (callback_) callback_(frame_.data(), frame_.size(), header.timestamp);
      }
      reset();
    }
  }
...
class Camera::Impl {
 public:
  ...
  int open(const DeviceInfo& wanted, RgbCallback rgb_cb, HqRgbCallback hq_rgb_cb, IrCallback ir_cb, DepthCallback depth_cb) {
    ...
    for (ssize_t i = 0; i < count; ++i) {
      ...
      if (desc.idVendor != hardware::kMicrosoftVid || desc.idProduct != hardware::kCameraPid) continue;
    ...
    // Match the Windows backend: discover the common video/depth alternate
    // setting from live descriptors. Interface 0 / alt 0 is the Xbox 360
    // layout used when descriptor discovery finds no better candidate.
    camera_interface_ = kCameraInterface;
    camera_alt_setting_ = 0;
    discover_camera_stream_interface();

    rc = libusb_claim_interface(handle_, camera_interface_);
    if (rc < 0) {
      close();
      return rc;
    }
    ...
    if (camera_alt_setting_ != 0) {
      rc = libusb_set_interface_alt_setting(handle_, camera_interface_, camera_alt_setting_);
      if (rc < 0) {
        close();
        return rc;
      }
  ...
  void close() {
    if (handle_ != nullptr) {
      ...
      (void)set_projector(false);
      if (claimed_) (void)libusb_release_interface(handle_, camera_interface_);
  ...
  int start_video(VideoMode mode) {
    ...
    video_mode_ = mode;
    rgb_assembler_.restart();
    rgb_hq_assembler_.restart();
    ir_assembler_.restart();

    // Match the proven Windows sequence: stop/configure while the engine is
    // idle, arm endpoint 0x81, and only then set register 0x05 to start the
    // stream. The old Linux path armed ISO first and immediately reset 0x05,
    // which could cancel the just-armed RGB/IR engine on both 1414 and 1473.
    int rc = write_register(0x05, 0x00);
    ...
    const int iso_rc = start_iso(video_stream_, kVideoEndpoint, kVideoPacketBytes,
    ...
    rc = write_register(0x05, mode == VideoMode::Infrared ? 0x03 : 0x01);
    if (rc >= 0) rc = write_register(mode == VideoMode::Infrared ? 0x48 : 0x47, 0x00);
    if (rc < 0) {
  ...
  // Match the proven Windows depth sequence: put the firmware depth path in a
  // known OFF state, configure 11-bit VGA at 30 fps, arm endpoint 0x82, and
  // only then enable the projector/depth engine (register 0x06). Configuring
  // the engine after arming ISO can reset a freshly queued endpoint session.
  int start_depth() {
    if (handle_ == nullptr) return LIBUSB_ERROR_NO_DEVICE;
    if (depth_running_) return 0;
    depth_assembler_.restart();

    int rc = write_register(0x105, 0x00);
    if (rc >= 0) rc = write_register(0x06, 0x00);
    if (rc >= 0) projector_on_ = false;
    ...
    const int iso_rc = start_iso(depth_stream_, kDepthEndpoint, kDepthPacketBytes,
                                  [this](const uint8_t* p, int n) { depth_assembler_.process(p, n); });
    if (iso_rc < 0) return iso_rc;

    rc = set_projector(true);
    if (rc < 0) {
      (void)set_projector(false);
  ...
  // Stops the firmware depth engine before endpoint 0x82 is cancelled. The
  // caller re-enables the projector afterwards when IR still needs it.
  int stop_depth() {
    if (!depth_running_ && !depth_stream_.started) return 0;
    int rc = 0;
    if (handle_ != nullptr) rc = set_projector(false);
    depth_running_ = false;
    stop_iso(depth_stream_);
  ...
  // Register 0x06 drives the IR projector. Like the Windows backend it is
  // reference counted by the caller: on while IR or Depth is requested.
  int set_projector(bool on) {
    if (handle_ == nullptr) return LIBUSB_ERROR_NO_DEVICE;
    if (projector_on_ == on) return 0;
    const int rc = write_register(0x06, on ? 0x02 : 0x00);
    if (rc >= 0) projector_on_ = on;
    return rc;
  }

  StreamStats video_stats() const {
    if (video_mode_ == VideoMode::Rgb) return rgb_assembler_.stats();
    if (video_mode_ == VideoMode::RgbHighQuality) return rgb_hq_assembler_.stats();
    return ir_assembler_.stats();
  }
  StreamStats depth_stats() const { return depth_assembler_.stats(); }
  ...
  int process_events(int timeout_ms) {
    ...
    return rc == LIBUSB_ERROR_INTERRUPTED ? 0 : rc;
  }
 ...
 private:
  ...
  void discover_camera_stream_interface() {
    ...
    if (libusb_get_active_config_descriptor(device, &config) != 0 || config == nullptr) {
      if (libusb_get_config_descriptor(device, 0, &config) != 0 || config == nullptr) return;
    }

    bool found = false;
    // Prefer interface 0, which is the camera interface in the proven
    // libfreenect layouts. A second pass allows a descriptor-correct firmware
    // variant to move the pair without hard-coding that assumption.
    for (int pass = 0; pass < 2 && !found; ++pass) {
      for (uint8_t i = 0; i < config->bNumInterfaces && !found; ++i) {
        ...
        for (int a = 0; a < interface.num_altsetting; ++a) {
          ...
          if (pass == 0 && alternate.bInterfaceNumber != kCameraInterface) continue;
          if (pass == 1 && alternate.bInterfaceNumber == kCameraInterface) continue;
          bool video = false;
          bool depth = false;
          for (uint8_t e = 0; e < alternate.bNumEndpoints; ++e) {
            ...
            if (endpoint.bEndpointAddress == kVideoEndpoint) video = true;
            if (endpoint.bEndpointAddress == kDepthEndpoint) depth = true;
          }
          if (!video || !depth) continue;
          camera_interface_ = alternate.bInterfaceNumber;
          camera_alt_setting_ = alternate.bAlternateSetting;
          found = true;
          break;
        }
      }
  ...
  void handle_iso_callback(IsoSlot& slot, IsoStream& stream, libusb_transfer* transfer) {
    if (transfer->status == LIBUSB_TRANSFER_NO_DEVICE ||
        (transfer->status == LIBUSB_TRANSFER_CANCELLED && !stream.stopping))
  ...
  int start_iso(IsoStream& stream, uint8_t endpoint, int fallback_packet_size,
    ...
    const int packet_size = negotiated > 0 ? negotiated : fallback_packet_size;
  ...
  void stop_iso(IsoStream& stream) {
    ...
    for (auto& slot : stream.slots) {
      ...
      // Never free a submitted libusb transfer until its completion callback
      // has run. Even NOT_FOUND can race an already queued completion.
    ...
    // This mirrors libfreenect's cancellation discipline: pump events until
    // every submitted transfer has retired, preventing callback user_data from
    // pointing at freed slots/buffers during reconnect or service shutdown.
  ...
  int send_command(uint16_t command, const uint8_t* payload, std::size_t payload_bytes,
    ...
    const uint16_t output_bytes = static_cast<uint16_t>(payload_bytes + 8u);
    int rc = libusb_control_transfer(handle_, 0x40, 0, 0, 0, output.data(), output_bytes, 1000);
    if (rc < 0) return rc;
    if (rc != output_bytes) return LIBUSB_ERROR_IO;

    std::array<uint8_t, 0x200> input{};
    int actual = 0;
    const auto deadline = std::chrono::steady_clock::now() +
        std::chrono::milliseconds(kControlReplyDeadlineMs);
    for (;;) {
      actual = libusb_control_transfer(handle_, 0xc0, 0, 0, 0, input.data(),
                                       static_cast<uint16_t>(input.size()), 250);
      if (actual < 0) {
        if (actual == LIBUSB_ERROR_TIMEOUT && std::chrono::steady_clock::now() < deadline) continue;
        return actual;
      }
      // Windows' proven transport treats a zero-length or full 0x200-byte read
      // as "reply not ready" and polls again within one bounded deadline.
      if (actual != 0 && actual != static_cast<int>(input.size())) break;
      if (std::chrono::steady_clock::now() >= deadline) return LIBUSB_ERROR_TIMEOUT;
      std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
    ...
    if (reply_bytes > static_cast<int>(reply_capacity)) return LIBUSB_ERROR_OVERFLOW;
    if (reply != nullptr && reply_bytes != 0) {
      std::memcpy(reply, input.data() + 8, static_cast<std::size_t>(reply_bytes));
    }
    ++camera_tag_;
    return reply_bytes;
  }
  ...
  int write_register(uint16_t reg, uint16_t value) {
    ...
    return rc < 0 ? rc : (rc >= static_cast<int>(sizeof(uint16_t)) ? 0 : LIBUSB_ERROR_IO);
  }
  ...
  bool fetch_depth_calibration() {
    ...
    // libfreenect documents 322/334-byte fixed-parameter replies; the Windows
    // backend only requires the calibrated fields we consume. Accept any sane
    // reply large enough to contain those fields so firmware variants remain
    // interoperable.
    if (rc < 110) return false;
    ...
    // Same acceptance as the Windows backend: the constant shift is the second
    // 16-bit word; firmware may append further words.
    std::array<uint8_t, 8> shift_reply{};
    rc = send_command(0x16, shift_command.data(), shift_command.size(), shift_reply.data(), shift_reply.size());
    if (rc < 4) return false;

    const uint16_t shift = get_le16(shift_reply.data() + 2);
    if (shift == 0) return false;
    zero_plane_ = zero;
    const_shift_ = static_cast<double>(shift);
    depth_calibrated_ = true;
    return true;
  }
...
std::vector<DeviceInfo> enumerate() {
  ...
  if (count >= 0) {
    for (ssize_t i = 0; i < count; ++i) {
      ...
      info.model = hardware::model_from_camera(desc.idProduct, desc.bcdDevice);
      info.ports.assign(ports.begin(), ports.begin() + n);
      // Every Kinect function is a child of the sensor's internal USB hub.
      // The final port component identifies the sibling function (camera/audio/motor),
      // so exclude it from the public device identity. The full port vector remains
      // in DeviceInfo and is still used to reopen this exact camera interface.
      const int physical_components = std::max(1, n - 1);
      std::ostringstream id;
      id << "usb-" << std::setfill('0') << std::setw(3) << static_cast<unsigned>(info.bus) << '-';
      for (int p = 0; p < physical_components; ++p) { if (p) id << '.'; id << static_cast<unsigned>(ports[static_cast<std::size_t>(p)]); }
      info.id = id.str();
      // Same display name as Windows: the model; the stable ID is a separate field.
      info.label = std::string("Kinect ") + hardware::model_name(info.model);  // model is always 1414/1473 for 02AE
...
const char* error_text(int rc) { return libusb_error_name(rc); }
...
int Camera::set_projector(bool on) { return impl_->set_projector(on); }
StreamStats Camera::video_stats() const { return impl_->video_stats(); }
StreamStats Camera::depth_stats() const { return impl_->depth_stats(); }
