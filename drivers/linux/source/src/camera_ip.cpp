...
std::vector<uint8_t> jpeg_bayer(const uint8_t* pixels, int width, int height, int quality) {
  ...
  while (compressor.next_scanline < compressor.image_height) {
    JSAMPROW row = rgb.data() + static_cast<size_t>(compressor.next_scanline) * width * 3u;
  ...
  while (run) {
    ...
    while (run && active_clients.load() > 0) {
      ...
      if (header.pixelFormat == scanner::PixelFormat::BayerGrbg8 &&
          ((header.mode == scanner::StreamMode::Rgb && header.payloadBytes == scanner::kRgbRawPayloadBytes) ||
           (header.mode == scanner::StreamMode::RgbHighQuality && header.payloadBytes == scanner::kRgbHqPayloadBytes))) {
        const int width = header.mode == scanner::StreamMode::RgbHighQuality ? scanner::kRgbHqWidth : scanner::kWidth;
        const int height = header.mode == scanner::StreamMode::RgbHighQuality ? scanner::kRgbHqHeight : scanner::kHeight;
