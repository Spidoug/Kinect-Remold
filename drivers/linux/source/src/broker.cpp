...
constexpr int kSdkClientIoTimeoutMs = 1000;
...
class MotorSession {
 public:
  ...
  int status(control::Reply& reply) {
    // Status remains non-blocking while the motor is moving.
  ...
  int tilt(int requested, control::Reply& reply) {
    ...
    if (kind_ == MotorKind::Audio1473) {
      ...
      const int prepare = prepare_control_locked();
  ...
  int led(int mode, control::Reply& reply) {
    if (!(mode == static_cast<int>(control::LedMode::Off) ||
          mode == static_cast<int>(control::LedMode::Green) ||
          mode == static_cast<int>(control::LedMode::Red) ||
          mode == static_cast<int>(control::LedMode::BlinkGreen))) return -EINVAL;
    std::lock_guard<std::mutex> guard(mutex_);
    return with_fresh_handle_retry_locked([&](control::Reply& out) {
      int rc = -ENODEV;
      ...
      if (rc == 0) {
        primed_ = true;
        out.transport = control::Transport::PhysicalMotor;
      }
      return rc;
    }, reply);
  }
  ...
  int prepare_camera(control::Reply& reply) {
    std::lock_guard<std::mutex> guard(mutex_);
    return with_fresh_handle_retry_locked([&](control::Reply& out) {
      const int rc = prepare_control_locked();
      if (rc == 0) out.transport = control::Transport::PhysicalMotor;
      return rc;
    }, reply);
  }
 ...
 private:
  bool ensure_open_locked() {
    ...
    // Resolve the hardware profile from the camera identity. Model 1414 uses
    // the 02B0 motor function; model 1473 uses the 02BB/02C3 control interface.
    ...
    primed_ = false;
    if (kind_ == MotorKind::Audio1473) {
      // Discover the active bulk control interface while leaving the USB Audio
      // capture interface owned by ALSA.
  ...
  int prepare_control_locked() {
    if (primed_) return 0;
    if (kind_ == MotorKind::Classic1414) {
      const int rc = set_led_1414_locked(static_cast<int>(control::LedMode::Green));
      if (rc == 0) primed_ = true;
      return rc;
    }
    if (kind_ == MotorKind::Audio1473) return prime_1473_locked();
    return -ENODEV;
  }
  ...
  int set_led_1473_locked(int mode) {
    ...
    int alt = 0;
    if (mode == static_cast<int>(control::LedMode::Off)) alt = 1;
    else if (mode == static_cast<int>(control::LedMode::Green)) alt = 3;
    else if (mode == static_cast<int>(control::LedMode::Red)) alt = 4;
    else if (mode == static_cast<int>(control::LedMode::BlinkGreen)) alt = 2;
    else return -EINVAL;

    if (alt == 3 && !was_primed) return 0;
    return alt_command_locked(kAltLed, alt);
  }
...
std::vector<std::string> read_device_rows() {
  std::vector<std::string> rows;
  std::ifstream input(kDeviceManifest, std::ios::binary);
  std::string line;
...
std::string first_field(const std::string& row) {
  const std::size_t tab = row.find('\t');
  return tab == std::string::npos ? row : row.substr(0, tab);
}

std::string dispatch_sdk(const std::string& raw) {
  ...
  const auto rows = read_device_rows();
  if (command == "LIST") {
  ...
  if (command.rfind("GET ", 0) == 0) {
    ...
    for (const auto& row : rows) if (first_field(row) == id) return row + "\n";
    return "ERR NOT_FOUND\n";
  }
  if (command.rfind("INDEX ", 0) == 0) {
    char* end = nullptr;
    const long index = std::strtol(command.c_str() + 6, &end, 10);
    if (end && *end == '\0' && index >= 0 && static_cast<std::size_t>(index) < rows.size()) {
...
void sdk_client(int fd) {
  if (!unixio::set_io_timeout(fd, kSdkClientIoTimeoutMs)) return;
  char request[1024]{};
  const ssize_t count = ::read(fd, request, sizeof(request) - 1u);
  if (count <= 0) return;
  const std::string response = dispatch_sdk(std::string(request, static_cast<std::size_t>(count)));
  (void)unixio::write_all(fd, response.data(), response.size());
}

bool listener_ready(int server) {
  pollfd descriptor{server, POLLIN, 0};
  const int rc = ::poll(&descriptor, 1, 250);
  if (rc <= 0) return false;
  return (descriptor.revents & POLLIN) != 0;
}

void sdk_server_loop(int server) {
  while (!stop_requested) {
    if (!listener_ready(server)) continue;
...
void client(int fd) {
  ...
  if (request.command == control::Command::Ping && device_id.empty()) {
    // An unscoped Ping tests broker liveness. A device-scoped Ping checks the
    // selected physical control path.
    reply.result = 0;
...
int main() {
  ...
  const int control_server = unixio::local_server_socket(kControlSocket, 16);
  if (control_server < 0) {
    std::perror("control socket");
    libusb_exit(usb_ctx);
    return 3;
  }
  const int sdk_server = unixio::local_server_socket(kSdkSocket, 8);
  if (sdk_server < 0) {
  ...
  std::thread sdk_thread(sdk_server_loop, sdk_server);

  while (!stop_requested) {
    if (!listener_ready(control_server)) continue;
    const int connection = ::accept4(control_server, nullptr, nullptr, SOCK_CLOEXEC);
    if (connection < 0) {
      if (stop_requested) break;
      if (errno == EINTR) continue;
      unixio::retry_sleep();
