...
// Mirrors MotionSampler in the Windows CameraBridge: polling runs only while
// the camera session is online and a frame consumer is active, uses the
// per-model cadence, and current() never returns a sample older than the
// model's stale window. Every published frame carries current() taken at
// frame completion.
class MotionSampler {
 public:
...
class CameraHub {
 public:
  ...
  int acquire(uint32_t mask, pid_t pid, int fd, uint64_t& token,
    ...
    // Match Windows ScannerPort ownership: the newest connection from the same
    // process supersedes stale handles from that process. This makes RGB <-> IR
    // tab/mode changes deterministic even when the old descriptor closes late.
 ...
 private:
  // Match the Windows ScannerPort latency policy: three RGBD pairs are enough
  // to absorb a short scheduler stall without turning it into visible catch-up.
  ...
  // Same policy as the Windows CameraBridge: a sensor that never completes an
...
std::string audio_ready_endpoint(const std::string& id) {
  return std::string(kAudioDirectory) + "/" + id + ".ready";
}
...
void write_manifest(const std::map<std::string, std::unique_ptr<CameraNode>>& nodes,
  ...
  const fs::path target = dir / "devices.tsv";
  {
    ...
    for (const auto& entry : nodes) {
      ...
      const bool audio_capture_ready = physically_present && endpoint_ready(audio_ready_endpoint(id));
      const bool raw_audio_ready = audio_capture_ready && endpoint_ready(audio);
      const bool control_audio_ready = audio_capture_ready && endpoint_ready(audio_control);
