...
// active. Every published frame carries the freshest valid motion sample.
class MotionSampler {
 public:
...
void prepare_physical_camera(const kinectusb::DeviceInfo& info, bool startup_open) {
  ...
  if (startup_open) {
    std::fprintf(stderr, "camera: physical control preparation unavailable for %s; continuing with the camera transport\n",
                 info.id.c_str());
  }
}
