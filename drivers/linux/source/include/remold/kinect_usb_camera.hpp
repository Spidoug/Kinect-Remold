...
// Isochronous framing counters of one stream since it was last started.
struct StreamStats {
  uint64_t packets = 0;      // packets with a valid RB header
  uint64_t frames = 0;       // complete frames delivered
  uint64_t sync_losses = 0;  // frames abandoned by framing/sequence errors
};
...
// Human-readable libusb error text for camera service logs.
const char* error_text(int rc);
