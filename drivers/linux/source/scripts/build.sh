...
# The 1473 exposes motor/LED control only after the Kinect audio runtime has
# booted. Prepare the same verified Microsoft UAC firmware used by Windows so
# Linux audio and 1473 control are complete immediately after installation.
...
install -m 0755 "$PROJECT_ROOT/drivers/linux/DIAGNOSE.sh" "$DIST_DIR/DIAGNOSE.sh"
install -m 0644 "$PROJECT_ROOT/VERSION" "$DIST_DIR/VERSION"
bash "$PROJECT_ROOT/drivers/linux/FINGERPRINT.sh" >"$DIST_DIR/BUILD-FINGERPRINT"
install -m 0644 "$SOURCE_ROOT/include/remold/protocol.hpp" "$SDK_DIR/protocol.hpp"
install -m 0644 "$SOURCE_ROOT/include/remold/device_registry.hpp" "$SDK_DIR/device_registry.hpp"

required_runtime=(
  "$DIST_DIR/bin/kinect360-remoldctl"
  "$DIST_DIR/libexec/kinect360-remold/kinect360-remold-sdk"
  ...
  "$DIST_DIR/KINECT.sh"
  "$DIST_DIR/DIAGNOSE.sh"
  "$DIST_DIR/VERSION"
  "$DIST_DIR/BUILD-FINGERPRINT"
  ...
  "$DIST_DIR/support/systemd/kinect360-remold-sdk.service"
...
Linux SDK socket: `/run/kinect360-remold/sdk.sock`.
