...
LINUX_FINGERPRINT="$ROOT/drivers/linux/FINGERPRINT.sh"
LINUX_BUILT_FINGERPRINT="$DRIVER_DIST/BUILD-FINGERPRINT"
...
required=(
  ...
  "$DRIVER_LIBEXEC/kinect360-remold-sdk"
  ...
  "$DRIVER_DIST/DIAGNOSE.sh"
  "$DRIVER_DIST/VERSION"
  "$DRIVER_DIST/BUILD-FINGERPRINT"
...
# The generated Linux runtime is used only when it was built from the current
# driver sources; the fingerprint covers every file under drivers/linux.
if (( ready )); then
  if [[ ! -r "$LINUX_BUILT_FINGERPRINT" ]] ||
     [[ "$(bash "$LINUX_FINGERPRINT" 2>/dev/null)" != "$(cat "$LINUX_BUILT_FINGERPRINT")" ]]; then
    ready=0
  fi
fi
...
# Fast path: generated runtime is complete, exactly like the Windows launcher.
